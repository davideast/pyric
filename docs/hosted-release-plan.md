...
| 3a Undo revert (engine) | `slice/undo-revert` at `1b1dfc54` (nine-file tree difference for `packages/pyric`, rules row keeps its null auth argument, `Conformance-Exempt:` trailer on the commit) | https://github.com/davideast/pyric/pull/655 opened 2026-09-19 | verified 2026-09-19 by the reviewer: pyric 7647 pass; pyric-admin 660; cli serve 1427 against the reverted engine; pyric, cli, Studio typechecks exit 0; compat regeneration no diff | squash-merged as `main` `2a942b93` on 2026-09-19 |
| 3 Host | `slice/host` at `7b3fea39` (two commits on `main` `2a942b93`: product commit `dc55162d` byte-equal to the two-argument tree difference for `packages/cli/src`, `packages/cli/test/serve`, `packages/cli/test/cli`, the persistence contract, and the release-contract fixture at `6fe34321`, 92 files; plus the handoff documents). Spec: `docs/hosted-extraction-spec-host.md`. Two earlier cuts were discarded: the first path list missed two files edited by later fixes, the second failed the snapshot fixture under CI's invocation (I15) | https://github.com/davideast/pyric/pull/656 opened 2026-09-20 at `1a0b7454`; every required check green on the first run, no follower commits | verified 2026-09-20 by the reviewer on the slice: all 17 mapped items pass; whole cli package under `bun test --cwd packages/cli` 3880 pass, 0 fail; cli, Studio, UI typechecks exit 0; parity check, coupling gate, conformance gates exit 0; served-app Playwright conformance 8 passed; hosted SQLite suite green on Node 22.15.0 and 22.18.0 (implementing agent) | squash-merged as `main` `40d1cd35` on 2026-09-20 |
| 3b Follow-ups (C1, C2, C4, C7, C8, C12, backlog contract) | `slice/followups` at `654cc3d7`, cut by the implementing agent per `docs/hosted-extraction-spec-followups.md`; product commit byte-equal to the four-path tree difference the reviewer had dry-run on `main` | https://github.com/davideast/pyric/pull/658 opened 2026-09-21; every required check green on the first run | verified 2026-09-21: cli package 3902 pass, 0 fail; 34 mapped items pass; cli and Studio typechecks, parity, both gates, libraries job, served-app conformance all pass | squash-merged as `main` `a3940fb9` on 2026-09-21 |
| 3c Engine event log bound (I16) | `slice/event-log-bound` at `6e17426f`, seven files in `packages/pyric`, cut and proven by the reviewer on `main` `40d1cd35` | https://github.com/davideast/pyric/pull/657 opened 2026-09-21; every required check green on the first run | verified 2026-09-21: engine acceptance 7 pass and all fail with pruning removed; pyric 7654, cli 3880, conformance 391, four typechecks, compat no diff, coupling gate exempt by trailer | squash-merged as `main` `9e0bd166` on 2026-09-21 |
| 4 Acceptance | | | | |
| 5 Studio and peel | | | | |
...
State at 2026-09-21: `main` `a3940fb9` carries every reviewed product change. `git diff origin/main origin/hosted-main-integration` is empty for `packages/pyric`, `packages/pyric-admin`, `packages/cli/src`, `packages/cli/test/{serve,cli,bridge,remote}`, and the persistence contract. What remains on the integration branch is phase 4 material (the hosted Playwright and manual suites, the evidence scripts and documents, the browser-boundary check, the support contract check) and phase 5 material (Studio, UI, playground, the example app, packaging and workflow changes).

Owner decision, 2026-09-22: stop adding scope. Every slice of product code is merged and hosted mode is flag-gated with no users, so evidence infrastructure for it protects nobody yet. The test applied to each proposed step was what goes wrong, for whom, and how soon, if it is skipped. Two things survive it and are the whole plan: (1) the served-app CI job uploads its Playwright trace and the serve process's stderr, so a recurrence of the popup sign-in hang (ledger F6) is diagnosable; (2) the owner tests hosted mode by hand on real projects, using `.agents/skills/pyric-node-host`. Deferred until a trigger occurs: the hosted smoke set in CI, the full hosted Playwright suite, sustained-load baselines and the resident-memory ceiling (I6, I17), the shard fix (F5), and the owed conformance captures (E1). Triggers: a decision to announce hosted mode, a user other than the owner, or work landing on the serve path often enough that a hosted regression would go unnoticed. Phase 5 does not start before one of those either.

State at 2026-09-22, after the owner's decision to stop adding scope: the two remaining changes are merged. Pull request 660 (`main` `2445806e`) carries the hosted launcher wording (ledger I18) and the browser conformance job's trace retention and results upload (the diagnosability half of ledger F6; the hang itself stays open as a watch item, and on a passing run the upload step reports no files, which is expected with traces retained only on failure). Pull request 661 (`main` `f6e30f35`) adds the repository-local skills `.agents/skills/pyric-node-host` and `.agents/skills/pyric-migrate-to-node-host`. Nothing is in flight and neither agent has a queue. The next input is the owner's manual testing of hosted mode on real projects; findings are filed in the ledger and routed through the review channel.

Correction, 2026-09-23: the statement above that nothing was in flight was wrong about Studio. Deferring phase 5 as a whole also deferred Studio's hosted client, without which Studio on `main` reconnects in a loop against a hosted server (ledger C15). The hosted client is cut on its own as `slice/studio-hosted`, nine files, leaving the traffic inspector redesign deferred with the rest of phase 5. The test for deferral stands; the phase boundary did not match it.
