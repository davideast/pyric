...
outcome: Write a rule, run a real request against it in the sandbox, read the verdict, and deploy knowing what it allows.
status: draft
---
...
This is not only a test-time thing. While `pyric dev` runs, your `firestore.rules` is loaded into the sandbox and hot-reloaded on save, and every operation your app performs carries this same verdict. See [read a denial and understand it](../secure/read-a-denial.md).
...
- [Write a rules test suite](../secure/write-a-rules-test-suite.md). Turn one-off checks into a suite that runs in CI.
...
- [Case studies](../secure/rules-case-studies.md). Deployed rulesets that enforce chess, connect four, and tax math.
