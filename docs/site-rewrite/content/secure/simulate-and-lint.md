...
The exact numbers behind the limit checks are in [the compiler and evaluator limits](../secure/limits-that-bite.md). To make simulation a habit rather than a one-off, [write a rules test suite](../secure/write-a-rules-test-suite.md).
