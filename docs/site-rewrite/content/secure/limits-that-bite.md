...
outcome: Know the measured limits of the production rules compiler and evaluator, with the exact numbers and the failing shape, so your rules compile the first time and hold up under load.
status: draft
---
...
The structural pattern this points to is covered in [rules patterns](../secure/rules-patterns.md).
...
Lint and simulate together in [simulate and lint before you deploy](../secure/simulate-and-lint.md). To see rules engineered against these exact limits, read the [case studies](../secure/rules-case-studies.md).
