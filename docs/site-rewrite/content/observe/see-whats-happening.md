...
An agent doesn't scroll a panel. It calls `sandbox_inspect` and gets the current rules, a lint summary, a document census, and the recent requests and denials in one response. That one call replaced a debugging session that once took fifty-one tool calls. See [Set up an agent](../agent/set-up-an-agent.md).

## Where to go next

Chase a specific denial in [Read a denial and understand it](../secure/read-a-denial.md), or start treating the state you're watching as source in [Shape your data](./shape-your-data.md).
