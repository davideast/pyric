...
- Severity: should-fix, watch. Slice: `evidence`. Status: open; artifact capture at verify (Codex, `work/integration`). Observed 2026-09-21 on `main` `a3940fb9`, the first commit to carry the bounded event log (pull request 657) and the follow-up fixes (pull request 658) together; the runs for the two preceding merge commits were cancelled by the next push, so the combination was first tested there.
