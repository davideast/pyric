...
- Severity: blocker for the transport pull request. Slice: `transport`. Status: verify (Codex, `work/integration`). Served-entry cases run unchanged in a fresh process; a preload acceptance reproduces the prior evaluation order. Local worker suite and A5 acceptance pass; CI second-shard proof remains required on the transport slice. Found by CI on the transport pull request, 2026-09-19.
