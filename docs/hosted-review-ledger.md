...
- Severity: should-fix. Slice: `core tools`. Status: closed on the shared branch, not yet on `main`; implemented by the reviewer after the implementing agent ran out of quota, so it has had no second reader; ruled. Owner ruling, 2026-09-21 ("Go with your recommendations"): connect the advertised action to the forwarded Auth outbox operation and keep it advertised; a failing acceptance through the real tool dispatcher comes first; the tool parity check applies; it lands on `main` as its own pull request after I20, whose operation it depends on.
...
- Fix, 2026-09-21: the rendered surface and the hosted host both execute method records against an in-process sandbox, so the action needs no worker port and no new bridge tool. `take_mail` routes to a new canonical operation `take_auth_mail`, served by the method record `auth.takeAuthMail`, which calls the engine's public outbox operation on the context's sandbox, the same function the worker's `auth.takeMail` delegates to. The CLI command `pyric auth takeAuthMail [--email]` is generated from the same record. Acceptance, committed failing first: through a real MCP session on the discriminator surface the action returns a message the application issued, removes it, filters by recipient case-insensitively, returns `mail: null` when nothing matches, and leaves stored accounts unchanged; in a hosted browser case a served page issues a sign-in link and the service CLI takes it from the host. That case, `test/e2e/auth-take-mail-hosted.pw.ts`, runs a Vite project with the plugin in Node host mode and is part of `test:identity-conformance`, because the hosted browser fixtures are not on `main`. Probe: with the route's selector changed, both dispatcher cases fail. cli package 3974 pass, 0 fail; cli and Studio typechecks clean; tool parity and coupling gate exit 0. The method list pins move with it: release contract, `sdk-names/pyric.json`, the MCP contract test, the handler suite's every-method rule, and the eval contract table. The auth tool's description stays within its 2,000 character budget, which set the wording of the method's one-line summary.
- Recorded, not pursued: the method's effect is `write`, so the hosted host requires healthy persistence and flushes after it, although the outbox is in memory and the worker operation is classed session-only. Over-gating a session-only operation is safe, as recorded under I20.
- Inventory of other advertised discriminator values with no route, found while here and not changed: `inspect_auth_flow` `verify_action_code` and `stage_oauth_mock`; `control_sandbox_environment` `set_network`; `verify_security_rules` `resolve_modules`. A call to one returns "this combination is not available in this build." `render.test.ts` deliberately keeps one such value as its example of an advertised value with no canonical counterpart; it now uses `stage_oauth_mock`. Whether to route or stop advertising each is the owner's call.
