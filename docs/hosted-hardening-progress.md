...
8. Malformed requests — in progress. Non-object, missing-type and unknown-type worker envelopes now reject the offending connection without an uncaught exception; another app remains usable and the affected app can reconnect. Correlation fields, remaining payloads, versions and size/depth boundaries remain open.
