...
State survives server restarts in this example's `.pyric/state/` directory.
To include Studio at http://localhost:5217/__pyric/ui/studio, build its embedded
assets with `bash scripts/build.sh` before starting the server; the packages-only
build skips those assets. Studio sees the same shared data.
...
public HTTPS URL instead to check the Tailscale HTTP route too.
