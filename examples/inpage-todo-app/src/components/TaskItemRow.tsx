...
export const TaskItemRow: React.FC<TaskItemRowProps> = ({
  ...
  return (
    <div
      className={`group flex items-center justify-between p-3.5 sm:px-4 sm:py-3 gap-3 border-b border-zinc-800 hover:bg-zinc-900/40 transition-colors ${
    ...
    >
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <button
          ...
          className={`w-5 h-5 rounded border border-zinc-700 flex items-center justify-center transition-colors shrink-0 ${
            task.completed
              ? 'bg-white border-white text-zinc-950 font-bold'
              : 'bg-transparent text-transparent hover:border-zinc-500'
          }`}
        >
          ✓
        </button>
        <div className="flex-1 min-w-0 flex flex-col gap-1">
          {isEditing ? (
            ...
            <div className="flex items-center flex-wrap gap-2">
              ...
              <span
                className={`px-1.5 py-0.5 rounded font-mono text-[10px] font-semibold border ${priorityBadgeClass}`}
              >
                {task.priority}
              </span>
