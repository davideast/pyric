...
try {
  ...
  console.error(
    '[playground-debug] playwright is not installed. Run:\n' +
      '  cd examples/playground-next && bun add -d playwright && bunx playwright install chromium',
  );
  process.exit(1);
}
  ...
  try {
    ...
    throw new Error(
      `Could not reach ${BASE_URL}. Is the dev server running? ` +
        `(cd examples/playground-next && bun --env-file=../../.env --env-file=.env astro dev --port 4329)\n` +
