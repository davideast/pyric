...
function locateEvent(
...
): { event: TrafficEntry; source: 'event-id' | 'latest-denial' } | null {
  ...
  for (let i = traffic.length - 1; i >= 0; i--) {
    if (traffic[i].result === 'deny') {
      return { event: traffic[i], source: 'latest-denial' };
    }
  }
  return null;
}
 ...
 */
function trafficEntryToTestCase(event: TrafficEntry): TestCase {
