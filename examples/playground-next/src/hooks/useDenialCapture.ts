...
function shrinkData<T extends Record<string, unknown> | null | undefined>(
  data: T,
): { value: T; truncated: boolean } {
  if (!data) return { value: data, truncated: false };
  try {
...
function shrinkRequestEvent(ev: RequestEvent): TrafficEntry {
  let truncated = false;
  const next: TrafficEntry = { ...ev };
  if (ev.request?.resourceData) {
    const out = shrinkData(ev.request.resourceData);
    next.request = { resourceData: out.value };
...
function describeOp(ev: RequestEvent): string {
  return `${ev.method} ${ev.path}`;
}

function describeAuth(ev: RequestEvent): string {
 ...
 */
function buildRequestShape(ev: RequestEvent): unknown {
  const request: Record<string, unknown> = {
    method: ev.method,
    path: ev.path,
  };
  if (ev.request?.resourceData !== undefined) {
    request.resource = { data: ev.request.resourceData };
  }
...
export function useDenialCapture(): void {
  ...
  useEffect(() => {
    const handleEvent = (event: { kind: string } | RequestEvent) => {
      ...
      pushTraffic(shrinkRequestEvent(ev));
