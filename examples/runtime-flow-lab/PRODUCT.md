...
Deliveries, directory records, and request entries are simulated local fixture data. React commits, fiber inspection, listener folding, and the production Flow painter are real. Correlation is not a traced data dependency. Heat represents observed marks, not CPU cost; dimensions represent geometry, not render duration; sequence numbers represent fixture deliveries. Sample portraits use Pyric's URL-backed avatar resolver and local cache.
