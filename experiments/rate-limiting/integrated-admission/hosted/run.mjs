...
const out = join(repo, 'experiments/rate-limiting/integrated-admission/hosted/results', measurementId);
...
const paths = [
    ...
    'fixtures/policy.mjs',
].map(name => `experiments/rate-limiting/integrated-admission/${name}`);
...
try {
    if (local) {
        ...
        const { createHostedIntegratedApp } = await import('./http-app.mjs');
        const service = createHostedIntegratedApp({
        ...
        if (report.project !== 'digame-mas' || report.service !== 'integrated-admission' || report.region !== 'us-east4'
        ...
        const token = execFileSync(process.env.GCLOUD_BIN || 'gcloud', [
            ...
            env: { ...process.env, CLOUDSDK_CONFIG: join(tmpdir(), 'integrated-cloudrun-gcloud'), CLOUDSDK_AUTH_CREDENTIAL_FILE_OVERRIDE: resolve(args[1]) },
    ...
    await visit(out);
    await save('manifest.json', { formatVersion: 1, captureKind: 'integrated-admission-hosted-workload', measurementId, local, credentialsBundled: false, files });
}
