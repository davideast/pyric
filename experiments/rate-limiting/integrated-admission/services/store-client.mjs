...
// Native transaction callbacks run in the gateway worker process.
// The parent (cluster) owns the in-process Pyric sandbox and reruns this
// callback when its transaction retries.
export function storeClient() {
    const pending   = new Map();   // id -> { resolve, reject }
    const callbacks = new Map();   // key -> callback fn
    ...
    process.on('message', async raw => {
        ...
        if (message.type === 'reply') {
            ...
            try {
                ...
                await call('commit', { attempt: message.attempt, error: error.code ?? error.message });
            }
        }
    });
    ...
    return {
        ...
        store: {
            ...
            async transaction(context, callback) {
                ...
                try   { return await call('transaction', { key, context }); }
                finally { callbacks.delete(key); }
            },
        },
    };
}
