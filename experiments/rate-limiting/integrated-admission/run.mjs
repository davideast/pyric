...
const ALLOWED_OPTIONS = ['--config', '--case', '--out'];
...
try {
    ...
    if (command === 'preflight') {
        // Read-only: check source paths exist, no writes
        ...
        result = await captureRun(
            option('--out') ?? '/tmp/integrated-admission-results',
        ...
        result = await captureRun(
            option('--out') ?? '/tmp/integrated-admission-replays',
