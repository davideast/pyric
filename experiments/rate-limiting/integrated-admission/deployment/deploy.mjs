...
const [deployerFile = join(homedir(), 'Documents/digame-mas.json')] = process.argv.slice(2);
const identity = JSON.parse(await readFile(resolve(deployerFile), 'utf8'));
if (identity.type !== 'service_account' || identity.project_id !== 'digame-mas')
    throw new Error('Expected digame-mas service account');
const runtimeEmail = 'firebase-adminsdk-fbsvc@digame-mas.iam.gserviceaccount.com';

const project = 'digame-mas', region = 'us-east4', service = 'integrated-admission', databaseId = 'allowance-experiments';
const runId = `integrated-${randomUUID()}`;
const configuration = join(tmpdir(), 'integrated-cloudrun-gcloud');
...
const database = json(['firestore', 'databases', 'describe', `--database=${databaseId}`]);
if (database.locationId !== region || database.type !== 'FIRESTORE_NATIVE' || database.databaseEdition !== 'STANDARD' || database.concurrencyMode !== 'PESSIMISTIC')
    throw new Error('Unexpected experiment database metadata');

const existing = json(['run', 'services', 'list', `--region=${region}`, `--filter=metadata.name=${service}`]);
if (existing.length) {
    if (existing[0].metadata?.labels?.purpose !== 'integrated-admission-experiment')
        throw new Error('Refusing to replace an unrelated service');
    if (publicBindings(json(['run', 'services', 'get-iam-policy', service, `--region=${region}`])))
        throw new Error('Refusing public service');
}
...
if (sink && (sink.destination !== destination || sink.filter !== filter || sink.disabled))
    throw new Error('Existing experiment log sink differs; review it before deployment');
if (!sink)
    command(['logging', 'sinks', 'create', sinkName, destination, `--log-filter=${filter}`, '--description=Pyric integrated admission experiment evidence']);
...
const report = {
    formatVersion: 1, status: 'prepared', project, region, service, runId,
    sourceHash: prepared.sourceHash, sourceFiles: prepared.files,
    deployerIdentity: identity.client_email, runtimeIdentity: runtimeEmail,
    ...
    logSink: { name: sinkName, destination, filter },
    workloadRun: 'not run', functionalSmoke: { status: 'pending' },
};
const save = () => writeFile(join(directory, 'deployment-report.json'), JSON.stringify(report, null, 2) + '\n');
await save();
console.log(JSON.stringify({ stage: 'prepared', directory, service, runId }));

try {
    command(['run', 'deploy', service, `--region=${region}`, `--source=${join(directory, 'source')}`,
        `--service-account=${runtimeEmail}`, `--build-service-account=projects/${project}/serviceAccounts/${identity.client_email}`,
        ...
        '--labels=purpose=integrated-admission-experiment',
        `--set-env-vars=GOOGLE_CLOUD_PROJECT=${project},INTEGRATED_DATABASE=${databaseId},INTEGRATED_RUN_ID=${runId}`]);
    ...
    if (publicBindings(iam) || deployed.metadata?.annotations?.['run.googleapis.com/invoker-iam-disabled'] === 'true')
        throw new Error('Private invocation verification failed');
    const revision = json(['run', 'revisions', 'describe', deployed.status.latestReadyRevisionName, `--region=${region}`]);
    Object.assign(report, {
        status: 'deployed', url: deployed.status.url, revision: deployed.status.latestReadyRevisionName,
        imageDigest: revision.status?.imageDigest ?? null, deployedAt: new Date().toISOString(),
    });
    await save();

    const target = new URL(report.url);
    if (target.protocol !== 'https:' || !target.hostname.endsWith('.run.app'))
        throw new Error('Unexpected Cloud Run URL');
    ...
    if (![401, 403].includes(unauthenticated.status))
        throw new Error('Unauthenticated invocation was not denied');

    const token = execFileSync('gcloud', ['auth', 'print-identity-token', `--audiences=${report.url}`, ...common], {
    ...
    if (metadata.runId !== runId || metadata.sourceHash !== report.sourceHash || metadata.revision !== report.revision)
        throw new Error('Deployment provenance mismatch');
    ...
    const response = await fetch(report.url + path, {
    ...
    report.functionalSmoke = {
        status: response.status === 200 && outcome.result?.record?.state === 'completed' ? 'passed' : 'failed',
    ...
    await save();
    if (report.functionalSmoke.status !== 'passed')
        throw new Error('Firestore smoke failed; deployment retained for diagnosis');
