...
async function load(path) {
    ...
    const state = Object.fromEntries(Object.entries(result.finalState).map(([key, value]) => [key, {
        ...
        queueEntries: value.queue?.length ?? 0,
...
const workload   = await compareSources(
    ...
    ['experiments/rate-limiting/fair-allocation/hosted/workload.mjs', ...sharedSource],
);
...
const report = {
    schemaVersion:   1,
    comparisonKind:  'paired HTTP fair-allocation workload (Pyric vs Cloud Run + Firestore)',
...
const out = resolve('experiments/rate-limiting/fair-allocation/hosted/analyses', randomUUID());
...
console.log(JSON.stringify({
    ...
    totalAssertions:    semantic.length,
    loggingStatus:      report.logging.collectionStatus,
}, null, 2));
if (!report.comparable || report.matchingAssertions !== semantic.length) process.exitCode = 1;
