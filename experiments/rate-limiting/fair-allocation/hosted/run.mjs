...
const out = join(repo, 'experiments/rate-limiting/fair-allocation/hosted/results', measurementId);
...
const paths = [
    ...
    'hosted/http-app.mjs',
    'hosted/provider.mjs',
    'architecture/immediate.mjs',
    'architecture/queue.mjs',
    'architecture/fifo.mjs',
    'architecture/round-robin.mjs',
    'fixtures/workloads.mjs',
].map(name => `experiments/rate-limiting/fair-allocation/${name}`);
...
try {
    if (local) {
        ...
        const { createHostedFairAllocationApp } = await import('./http-app.mjs');
        const service = createHostedFairAllocationApp({
        ...
        if (report.project !== 'digame-mas' || report.service !== 'fair-allocation' || report.region !== 'us-east4'
        ...
        const token = execFileSync(process.env.GCLOUD_BIN || 'gcloud', [
            ...
            env: { ...process.env, CLOUDSDK_CONFIG: join(tmpdir(), 'fair-cloudrun-gcloud'), CLOUDSDK_AUTH_CREDENTIAL_FILE_OVERRIDE: resolve(args[1]) },
    ...
    if (local) {
        ...
        const capture = await captureLogs({
            ...
            measurementId,
            documentCollection: 'fairAllocationExperiments',
    ...
    await visit(out);
    await save('manifest.json', { formatVersion: 1, captureKind: 'fair-allocation-hosted-workload', measurementId, local, credentialsBundled: false, files });
}
