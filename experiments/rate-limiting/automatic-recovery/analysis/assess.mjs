...
export function assess(result) {
    ...
    for (const row of result.assertions ?? []) {
        ...
        if (row.passed !== expectedPass) {
            ...
            if (row.name.includes('released') || row.name.includes('completed') || row.name.includes('settles'))
                livenessFailures++;
            else
                safetyViolations++;
        }
    ...
    if ((result.cases?.length ?? 0) !== ids.length)
        issues.push('Extra or missing case');

    const contractLimitedCases = (result.cases ?? []).filter(c => c.contractLimited).map(c => c.id);

    return {
        successfulExperiment:    issues.length === 0,
        safety:                  safetyViolations === 0,
        conditionalLiveness:     livenessFailures === 0,
        evidenceCompleteness:    issues.length === 0,
        contractLimitedCases,
        issues,
    ...
    return {
        ...
        cases: Object.fromEntries((result.cases ?? []).map(row => {
            ...
            return [row.id, {
                ...
                killedWorkers:          events.filter(e => e.kind === 'fault-worker-kill').length,
            }];
        })),
