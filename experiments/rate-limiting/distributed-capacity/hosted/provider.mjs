// Durable simulated work, not an AI provider. No timer or local process owns its
// lifetime: only an explicit fixture command can record completion/cancellation.
    ...
    return {
        async start(key, uid) {
            ...
            const job = await store.transaction({ ...context, phase: 'fixture-provider' }, async tx => {
                ...
                tx.put(ref(key), job); return job;
            });
            record('fixture-provider-start', { key, uid }); return job;
        },
        async observe(key) {
        ...
        async finish(key, state) {
            ...
            record('fixture-provider-terminated', { key, state }); return { key, state };
        },
    };
}
