...
async function load(path) {
    ...
    const events = manifest.local ? await json(join(path, 'server-events.json'))
    ...
    const state = Object.fromEntries(Object.entries(result.finalState).map(([key, value]) => [key, {
        active: value.capacity?.find(row => row.id === 'global')?.data.active,
        reservations: value.requests?.length, providerJobs: value.provider?.length,
        states: value.requests?.reduce((counts, row) => ({ ...counts, [row.data.state]: (counts[row.data.state] ?? 0) + 1 }), {}),
    }]));
    const assessment = { manifestVerified: true, successful: result.successful, cases: result.cases.length,
        assertions: result.assertions.length, recomputedAssertionsPass: result.assertions.every(row => isDeepStrictEqual(row.actual, row.expected) && row.passed),
        commands: attempts.length, terminalTransportErrors: attempts.filter(row => row.transportError).length,
        windowIncludesAllCompletedAttempts: attempts.every(row => row.endedAt && row.endedAt <= result.run.endedAt),
        instances: result.observedInstances.length, elapsedMs: Date.parse(result.run.endedAt) - Date.parse(result.run.startedAt),
        httpLatencyMs: { p50: p(.5), p95: p(.95), max: p(1) }, transactionAttempts: tx.length, transactionInvocations: invocations.size,
        extraTransactionAttempts: tx.length - invocations.size, fixtureStartAttempts: events.filter(row => row.kind === 'fixture-provider-start-attempt').length,
        routingRecoveries: result.routingRecoveries.length, state };
    return { path, result, manifestSha256: sha(await readFile(join(path, 'manifest.json'))), assessment };
}
const local = await load(localPath), hosted = await load(hostedPath);
...
const workload = await compareSources(join(localPath, 'source'), join(hostedPath, 'source'),
    ['experiments/rate-limiting/distributed-capacity/hosted/workload.mjs', ...sharedSource]);
const semantic = local.result.assertions.map(left => {
    ...
    return { caseId: left.caseId, name: left.name, local: left.actual, hosted: right?.actual,
...
const report = { schemaVersion: 1, comparisonKind: 'paired HTTP capacity state-machine workload', createdAt: new Date().toISOString(),
    captures: [local, hosted].map(row => ({ path: row.path, manifestSha256: row.manifestSha256, assessment: row.assessment })),
    deployedSource: deployed, architecture, workload,
    comparable: architecture.identical && workload.identical && local.assessment.recomputedAssertionsPass && hosted.assessment.recomputedAssertionsPass
        && local.assessment.successful && hosted.assessment.successful && semantic.length === hosted.result.assertions.length,
    matchingAssertions: semantic.filter(row => row.match).length, semantic,
    logging: { collectionStatus: capture.collectionStatus, coverage: capture.coverage,
...
const out = resolve('experiments/rate-limiting/distributed-capacity/hosted/analyses', randomUUID());
...
const files = await Promise.all(['comparison.json', 'analyze.mjs', 'evidence.mjs'].map(async path => ({ path, sha256: sha(await readFile(join(out, path))) })));
await writeFile(join(out, 'manifest.json'), JSON.stringify({ files }, null, 2) + '\n');
console.log(JSON.stringify({ out, comparable: report.comparable, matchingAssertions: report.matchingAssertions,
    local: local.assessment, hosted: hosted.assessment, logging: report.logging }, null, 2));
