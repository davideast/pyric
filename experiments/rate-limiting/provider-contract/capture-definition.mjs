...
export function captureDefinition() {
    const sourcePaths = ['execute.mjs', 'run.mjs', 'capture-definition.mjs', 'README.md', 'LIVE-PLAN.md'].map(name => `${experiment}/${name}`);
    ...
    return { experiment, sourcePaths: sourcePaths.sort(), recoverOnFailure: true,
        artifacts: ['result.json', 'assessment.json', 'workload.json', 'firestore.rules', 'findings.md', 'events.ndjson', 'summary.json', 'environment.json', 'capabilities.json', 'contract-profile.json', 'provider-observations.ndjson', 'attempts.ndjson', 'snapshots.json'],
