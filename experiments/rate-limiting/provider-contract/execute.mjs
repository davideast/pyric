...
const workload = { revision: 1, fixtureVersion, ...config, policy: 'terminal-evidence-v1', faultModel: 'independent provider process, gateway SIGKILL, explicit transport aborts, controlled terminal ordering' };
const rules = await readFile(new URL('./architecture/firestore.rules', import.meta.url), 'utf8');
const metadata = { id: process.env.PYRIC_EXPERIMENT_RUN_ID, selectedCases: config.cases, workloadHash: digest(JSON.stringify(workload)), rulesHash: digest(rules), implementationHash: implementationHash(),
    environment: { ...pyricBackend.environment, provider: 'controlled HTTP fixture', gateway: 'Node child process; controller IPC ingress', transactions: 'Pyric Admin SDK; client rules bypassed', realInference: false } };
