import { runPairedOwnership, pairedAssertions } from './paired-ownership.mjs';
import { runContention, contentionAssertions } from './contention.mjs';
...
export async function runExperiment(driver = pyricBackend) {
  ...
  const result = {
    ...
    run: {
      ...
      implementationHash: hash(['./harness.mjs', './claim-lifecycle.mjs', './contention.mjs', './paired-ownership.mjs', '../architecture/paired-claims.mjs', '../architecture/pixels.mjs', '../architecture/claims.mjs', '../architecture/firestore.rules', '../fixtures/workload.mjs', '../../../shared/evidence/compare.mjs'].map(path => readFileSync(new URL(path, import.meta.url), 'utf8'))),
  ...
  await runContention({ sdk: driver.sdk, backend, runCase, operation, assert, record });
  await runPairedOwnership({ sdk: driver.sdk, backend, runCase, operation, assert, record });
...
export function assessRun(result) {
  ...
  for (const caseId of expectedCases) {
    ...
    for (const name of expectedAssertions[caseId]) {
      ...
      const expectedPass = !((caseId === 'stale-grid' && name === 'distinct edits survive') || (caseId === 'claim-aba-control' && name === 'old claim write rejected after reacquisition') || (caseId === 'multiple-colors' && name === 'one user owns at most one color'));
      if (rows.length !== 1 || rows[0].passed !== expectedPass) issues.push(`${caseId}: ${name}`);
    }
  }
  return { successfulExperiment: issues.length === 0, issues, expectedNegativeControls: ['stale-grid: distinct edits survive', 'claim-aba-control: old claim write rejected after reacquisition', 'multiple-colors: one user owns at most one color'] };
}
