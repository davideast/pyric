...
# `pyric firestore rules lint` from the installed bin — exercises the rules grammar asset
...
RULES
if ( cd "$SMOKE" && "$PYRIC_BIN" firestore rules lint firestore.rules >/dev/null 2>&1 ); then
  echo "  ✓ pyric firestore rules lint parsed a ruleset (grammar asset via the published CLI)"
else
  echo "  ✗ pyric firestore rules lint failed from the installed bin"; exit 1
fi
