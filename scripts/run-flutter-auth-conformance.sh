...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  ...
  echo "Runs the Flutter Auth conformance test suite inside an isolated ARM64 Docker container."
  echo "Defaults:"
  echo "  suite_path: packages/flutter-client/test/auth/conformance_test.dart"
  echo "  xml_output_path: .conformance/results/auth-flutter.xml"
  exit 0
fi

SUITE="${1:-packages/flutter-client/test/auth/conformance_test.dart}"
XML_OUT="${2:-$ROOT/.conformance/results/auth-flutter.xml}"
...
if command -v container >/dev/null 2>&1; then
  ...
  container run --rm \
    ...
    bash -c '
      ...
      (flutter test --reporter=json "/workspace/$1" 2>&1 || true) | dart run tool/json_to_junit.dart auth-flutter > /tmp/out.xml && mv /tmp/out.xml "/out/$2"
  ...
  flutter pub get >/dev/null 2>&1 || true
  (flutter test --reporter=json "$HOST_SUITE" 2>&1 || true) | dart run tool/json_to_junit.dart auth-flutter > "$XML_DIR/$XML_NAME"
