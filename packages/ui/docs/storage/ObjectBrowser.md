...
- **Denied rows stay clickable** — the gate stamp is an early warning, not a block; clicking a denied folder surfaces the real `storage/unauthorized` through the `error` prop path. On prod handles verdicts are advisory (see [rules-aware affordances](./rules-aware-affordances.md)).

## See also

- [`<PathBreadcrumb>`](./PathBreadcrumb.md) — the ancestor trail above the browser.
- [`<VirtualList>`](../primitives/VirtualList.md) — the underlying virtualizer.
- [Rules-aware affordances](./rules-aware-affordances.md) + [`useStorageRulesGate`](./useStorageRulesGate.md) — the `gate` prop's source.
