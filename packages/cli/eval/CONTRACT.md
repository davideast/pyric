...
Effect enforcement (ADR-0014 Decision 5) runs once, in `method-validation.ts`'s `validateArguments`, which both the MCP dispatch path and `pyric <tool> <method>` call. A `destructive` method (today only `sandbox.reset`) is refused unless `args.confirm === true`; the refusal is an ordinary `InvalidArguments` rejection naming the field `confirm`. A `production` method is not mounted: it is absent from `tools/list`, `describe` does not answer for it, and a call naming it is refused, unless the headless server was started with `--allow-production` (or `PYRIC_ALLOW_PRODUCTION` set to a non-empty, non-`0`, non-`false` value, with the flag winning). The eval never sets either, so a `production` method never mounts in a run.
