 ...
 * (a pure function of @pyric/cli' wrapper entries + the bundled `pyric`
...
const playgroundUi = join(DIST, 'serve', 'playground-ui');
...
const playgroundBlob = existsSync(playgroundUi) ? collectTree(playgroundUi) : {};
...
process.stdout.write(
  `  embedded ${Object.keys(sdkBlob).length} SDK file(s), ${Object.keys(studioBlob).length} studio file(s), ` +
    `${Object.keys(playgroundBlob).length} playground file(s), ${Object.keys(docsBlob).length} docs file(s) ` +
...
writeFileSync(join(BUILD, 'embedded-playground.js'), banner + `export default ${JSON.stringify(playgroundBlob)};\n`);
...
writeFileSync(
  ...
  banner +
    ...
    `  playground: () => import('./embedded-playground.js').then((m) => m.default),\n` +
