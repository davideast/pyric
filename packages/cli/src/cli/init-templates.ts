...
- **Graduate:** fill \`.env\` from the Firebase console and point the config in
  \`public/app.js\` at it, then \`npx firebase deploy --only firestore:rules\` /
  \`npx firebase deploy --only hosting\`. Bare \`firebase/*\` imports need a bundler
...
// \`firebase deploy --only firestore:rules\`, then run the SAME code against the real backend:
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    // works out of the box. **Tighten these before \`firebase deploy --only firestore:rules\`
...
3. Deploy them: add a \`.firebaserc\`
   ...
   \`GOOGLE_APPLICATION_CREDENTIALS\`, then \`firebase deploy --only firestore:rules\`.
...
export const TEMPLATES: Record<'web' | 'node' | 'static', ScaffoldTemplate> = {
  ...
  static: {
    ...
    dependencies: { firebase: '^12.12.0' },
    devDependencies: { '@pyric/cli': '*', 'firebase-tools': '*' },
    dirs: ['public'],
    files: (name) => [
