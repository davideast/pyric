  ...
  for (const result of results) {
    ...
    const serviceSummary = Object.values(result.result.services)
      ...
      .map((service) => {
        ...
        return service.ok
          ? `${publicServiceName(service.service)}: ok (${info} informational)`
          : `${publicServiceName(service.service)}: ${failures} failure(s)`;
      })
      .join(', ');
    ...
    for (const service of Object.values(result.result.services)) {
      ...
      for (const divergence of failingDivergences(service.divergences)) {
        lines.push(`    [${publicServiceName(service.service)}] ${formatDivergence(divergence)}`);
      }
    }
  }
  return lines.join('\n');
  ...
  if (json) {
    process.stdout.write(
      JSON.stringify({
        ok: allOk,
        engines,
        rulesPaths: publicServiceRecord(rulesResolution.paths),
        results: results.map((result) => ({
          name: result.name,
          description: result.description,
          ok: result.ok,
          services: publicServiceRecord(result.result.services),
        })),
      }) + '\n',
    );
  } else {
    ...
    if (!allOk) {
      ...
      const rules = Object.entries(rulesResolution.paths)
        .map(([service, path]) => `${publicServiceName(service as VerifiableService)}=${basename(path)}`)
        .join(', ');
      process.stderr.write(
...
function selectedServices(flag: FlagValue | undefined, fixtures: Fixture[]): VerifiableService[] {
  ...
  if (union.size === 0) {
    throw new Error('fixture does not contain firestore or database rules services.');
  }
  return [...union].sort();
}
...
function parseRulesOverrides(flag: FlagValue | undefined): Map<VerifiableService, string> {
  ...
  for (const raw of flagStrings(flag)) {
    ...
    if (eq <= 0 || eq === raw.length - 1) {
      throw new Error(`--rules must use service-qualified values like --rules database=database.rules.json.`);
    }
...
function toVerifiableService(raw: string): VerifiableService {
  if (raw === 'firestore') return raw;
  if (raw === 'database') return 'rtdb';
  throw new Error(`unsupported verify service '${raw}'. Supported services: firestore, database.`);
}
...
function publicServiceName(service: VerifiableService): 'firestore' | 'database' {
  return service === 'rtdb' ? 'database' : 'firestore';
}

function publicServiceRecord<T>(
  record: Partial<Record<VerifiableService, T>>,
): Partial<Record<'firestore' | 'database', T>> {
  return {
    ...(record.firestore !== undefined ? { firestore: record.firestore } : {}),
    ...(record.rtdb !== undefined ? { database: record.rtdb } : {}),
  };
}
