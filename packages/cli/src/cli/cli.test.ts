...
describe('scanForInlinedFirebase', () => {
  ...
  it('flags an inlined Cloud Storage endpoint (catalog beyond auth/firestore)', async () => {
    const { scanForInlinedFirebase } = await import('./serve.js');
    const { mkdtempSync, mkdirSync, writeFileSync } = await import('node:fs');
    const { tmpdir } = await import('node:os');
    const { join } = await import('node:path');
    const dir = mkdtempSync(join(tmpdir(), 'pyric-inline-storage-'));
    mkdirSync(join(dir, 'assets'));
    writeFileSync(
      join(dir, 'assets', 'app-9f3.js'),
      'const u=`https://firebasestorage.googleapis.com/v0/b/${b}/o/${encodeURIComponent(p)}`;',
    );
    expect(scanForInlinedFirebase(dir)).toEqual(['assets/app-9f3.js']);
  });

  it('flags inlined Functions, FCM, Vertex AI and legacy RTDB hosts', async () => {
    const { scanForInlinedFirebase } = await import('./serve.js');
    const { mkdtempSync, writeFileSync } = await import('node:fs');
    const { tmpdir } = await import('node:os');
    const { join } = await import('node:path');
    const cases: Array<[string, string]> = [
      ...
      ['ai.js', 'fetch("https://aiplatform.googleapis.com/v1/publishers/google/models")'],
      ['rtdb.js', 'new WebSocket("wss://demo.firebaseio.com/.ws?v=5")'],
    ];
    for (const [name, body] of cases) {
      const dir = mkdtempSync(join(tmpdir(), 'pyric-inline-hosts-'));
      writeFileSync(join(dir, name), body);
      expect(scanForInlinedFirebase(dir)).toEqual([name]);
    }
  });

  it('always flags the GCP metadata IP', async () => {
    const { scanForInlinedFirebase } = await import('./serve.js');
    const { mkdtempSync, writeFileSync } = await import('node:fs');
    const { tmpdir } = await import('node:os');
    const { join } = await import('node:path');
    const dir = mkdtempSync(join(tmpdir(), 'pyric-inline-metadata-'));
    writeFileSync(join(dir, 'creds.js'), 'fetch("http://169.254.169.254/computeMetadata/v1/token")');
    expect(scanForInlinedFirebase(dir)).toEqual(['creds.js']);
  });

  it('scans .cjs bundles (CommonJS backend output)', async () => {
    const { scanForInlinedFirebase } = await import('./serve.js');
    const { mkdtempSync, writeFileSync } = await import('node:fs');
    const { tmpdir } = await import('node:os');
    const { join } = await import('node:path');
    const dir = mkdtempSync(join(tmpdir(), 'pyric-inline-cjs-'));
    writeFileSync(
      join(dir, 'server.cjs'),
      'fetch("https://firestore.googleapis.com/v1/projects/p/databases/(default)/documents")',
    );
    expect(scanForInlinedFirebase(dir)).toEqual(['server.cjs']);
  });

  it('scans explicit backend build dirs, including dot-dirs like .next/server', async () => {
    const { scanForInlinedFirebase } = await import('./serve.js');
    const { mkdtempSync, mkdirSync, writeFileSync } = await import('node:fs');
    const { tmpdir } = await import('node:os');
    const { join } = await import('node:path');
    const root = mkdtempSync(join(tmpdir(), 'pyric-inline-backend-'));
    ...
    const hits = scanForInlinedFirebase(root, {
      dirs: ['dist', '.next/server', 'functions', 'does-not-exist'],
    });
    expect(hits.sort()).toEqual(['.next/server/chunk.js', 'dist/server.cjs']);
  });

  it('leaves default (publicDir-only) behavior unchanged when no dirs are given', async () => {
    const { scanForInlinedFirebase } = await import('./serve.js');
    const { mkdtempSync, mkdirSync, writeFileSync } = await import('node:fs');
    const { tmpdir } = await import('node:os');
    const { join } = await import('node:path');
    const root = mkdtempSync(join(tmpdir(), 'pyric-inline-default-'));
    mkdirSync(join(root, '.next', 'server'), { recursive: true });
    writeFileSync(
      join(root, '.next', 'server', 'chunk.js'),
      'fetch("https://firestore.googleapis.com/v1/projects")',
...
describe('google endpoint catalog', () => {
  it('is a single shared table the scanner and net-guard can both read', async () => {
    const { GOOGLE_ENDPOINT_CATALOG } = await import('../google-endpoints.js');
    const hosts = GOOGLE_ENDPOINT_CATALOG.map((e) => e.host);
    for (const h of [
  ...
  it('excludes generic hosts from the build-artifact fingerprint set', async () => {
    const { GOOGLE_ENDPOINT_HOSTS, INLINE_FINGERPRINT_HOSTS } = await import(
      '../google-endpoints.js'
    );
    // Raw GCS is guard-relevant but appears in apps that never touch the SDK
    // (a public asset URL), and the scanner fails a build hard on a hit.
    expect(GOOGLE_ENDPOINT_HOSTS).toContain('storage.googleapis.com');
    expect(INLINE_FINGERPRINT_HOSTS).not.toContain('storage.googleapis.com');
    expect(INLINE_FINGERPRINT_HOSTS).toContain('firebasestorage.googleapis.com');
  });

  it('does not fail a build over a public Cloud Storage asset URL', async () => {
    const { scanForInlinedFirebase } = await import('./serve.js');
    const { mkdtempSync, writeFileSync } = await import('node:fs');
    const { tmpdir } = await import('node:os');
    const { join } = await import('node:path');
    const dir = mkdtempSync(join(tmpdir(), 'pyric-inline-gcs-'));
    writeFileSync(
      join(dir, 'hero.js'),
      'const logo = "https://storage.googleapis.com/my-bucket/logo.png";',
    );
    expect(scanForInlinedFirebase(dir)).toEqual([]);
  });

  it('matches hostnames by suffix, most specific first', async () => {
    const { lookupGoogleEndpoint } = await import('../google-endpoints.js');
    expect(lookupGoogleEndpoint('firebasestorage.googleapis.com')?.service).toContain('Storage');
    ...
    // Suffix matching must respect label boundaries: not a subdomain of the entry.
