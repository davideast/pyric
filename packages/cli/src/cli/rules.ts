...
import { resolveModules, type ResolveResult } from 'pyric/rules/internal/node';
import {
...
export interface RulesDeps {
  ...
  resolveModules?: (source: string, options?: { basePath?: string }) => ResolveResult;
  extractIndexes?: (paths: string[], queryVarName?: string) => ExtractResult;
}
...
export async function runFirestoreRulesResolve(
  parsed: ParsedArgs,
  deps: RulesDeps = {},
): Promise<number> {
  const out = deps.stdout ?? process.stdout;
  ...
  if (!sourcePath) {
    err.write('pyric firestore rules resolve: missing rules-file path. Usage: pyric firestore rules resolve <path> [--out <path>]\n');
    return 1;
  }

  let source: string;
  const absoluteSource = resolvePath(cwd, sourcePath);
  try {
    source = await (deps.readFile ?? readFile)(absoluteSource, 'utf-8');
  } catch (e) {
    err.write(`pyric firestore rules resolve: ${e instanceof Error ? e.message : String(e)}\n`);
    return 1;
  }

  const result = (deps.resolveModules ?? resolveModules)(source, {
    basePath: dirname(absoluteSource),
  });
  if (!result.success) {
  ...
  await (deps.writeFile ?? writeFile)(outputPath, result.data.resolved, 'utf-8');
...
export async function runFirestoreIndexesGenerate(
  ...
  if (paths.length === 0) {
    err.write('pyric firestore indexes generate: missing source path. Usage: pyric firestore indexes generate <path...> [--out <path>]\n');
    return 1;
  }

  const queryVarFlag = parsed.flags.get('query-var');
  const queryVarName = typeof queryVarFlag === 'string' ? queryVarFlag : undefined;
  const result = deps.extractIndexes
    ? deps.extractIndexes(paths, queryVarName)
    : new ExtractFirestoreIndexesHandler().execute({
        paths: paths.map((path) => resolvePath(cwd, path)),
  ...
  const outputPath = resolvePath(
    cwd,
    typeof outFlag === 'string' ? outFlag : 'firestore.indexes.json',
  );
  await (deps.mkdir ?? mkdir)(dirname(outputPath), { recursive: true });
  ...
  for (const warning of result.data.warnings) {
    err.write(`pyric firestore indexes generate: warning: ${warning.message}\n`);
  }
  return 0;
}
