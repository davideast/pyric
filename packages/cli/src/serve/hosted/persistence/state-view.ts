...
import type { StateStore, PyricStateFile } from '../../state-store.js';
...
export function createHostedStateView(projectDir: string, directory: string, database: Database, namespace: string): StateStore {
    ...
    writeSection(section, value) {
      ...
      database.commit(() => {
        for (const object of objects) {
          ...
          database.storage.putBytes(object.metadata.fullPath, bytes, object.blobType, object.metadata);
        }
      });
    },
  };
