...
type Counts = { reads: number; writes: number; deliveries: number; deletes: number; uploadedBytes?: number; downloadedBytes?: number; requests?: number; failures?: number; inputTokens?: number; outputTokens?: number; estimatedTokens?: number; unknownUsage?: number };
const keys = ['reads', 'writes', 'deliveries', 'deletes'] as const;
const visibleKeys = (service: string) => service === 'ai' ? ['requests', 'failures'] as const : service !== 'rtdb' ? ['reads', 'writes', 'deletes'] as const : ['reads', 'writes', 'deliveries'] as const;
const labels = { uploadedBytes: 'Uploaded bytes', downloadedBytes: 'Downloaded bytes', requests: 'Requests', failures: 'Failures', inputTokens: 'Backend input tokens', outputTokens: 'Backend output tokens', estimatedTokens: 'Estimated / scripted tokens', unknownUsage: 'Unknown usage', reads: 'Reads', writes: 'Writes', deliveries: 'Deliveries', deletes: 'Deletes' };
...
function pointsFor(methods: readonly SdkMethodRate[], from: number, to: number, service: SdkServiceRate, retained = false): Array<Counts & { second: number }> {
  ...
  return Array.from({ length: Math.max(1, to - from + 1) }, (_, index) => {
    ...
    if (service.service === 'ai') {
      ...
      return { ...counts, requests: indexedMethods.reduce((sum, method) => sum + (method.buckets.get(second)?.calls ?? 0), 0), failures: bucket?.aiFailures ?? 0, inputTokens: bucket?.aiInputTokens ?? 0, outputTokens: bucket?.aiOutputTokens ?? 0, estimatedTokens: bucket?.aiEstimatedTokens ?? 0, unknownUsage: bucket?.aiUnknownUsage ?? 0 };
    }
...
function active(point: Counts) { return (point.failures ?? 0) > 0 || (point.unknownUsage ?? 0) > 0 || (point.requests ?? 0) > 0 || (point.inputTokens ?? 0) > 0 || (point.outputTokens ?? 0) > 0 || (point.estimatedTokens ?? 0) > 0 || keys.some(key => point[key] > 0) || (point.uploadedBytes ?? 0) > 0 || (point.downloadedBytes ?? 0) > 0; }
...
export function createRateHistory(serviceName = 'rtdb') {
  ...
  function view(snapshot: SdkRateSnapshot): HistoryFrame | undefined {
    ...
    if (service.service === 'ai') for (const key of ['requests', 'failures', 'inputTokens', 'outputTokens', 'estimatedTokens', 'unknownUsage'] as const) {
