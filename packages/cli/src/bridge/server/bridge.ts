 ...
 *
 * Every data-plane tool call is forwarded over WS to the browser.
 * The bridge acts as a relay with a tool-name allow-list pinned by
 * the peer's `hello`; local rules tools execute in-process in the
 * standalone server.
 */
...
export interface Bridge {
   ...
   * the in-process tool wrapper in `mcp.ts` after a local tool
   * finishes. Sandbox-forwarded
