...
export default {
  ...
  description: 'Write one document, replacing it unless options.merge is true.',
  args: z.object({
  ...
  validate: (args, { fail }) => checkDocumentPath('setDoc', args, fail),
  async handler(args, ctx) {
    const options = (args.options ?? {}) as Args;
    const call = { path: args.path, data: args.data };
