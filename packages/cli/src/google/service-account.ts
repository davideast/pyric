/**
 * `fromServiceAccount` — build project access from a Google service
 ...
 *
 * Node-only: the function requires `node:fs/promises` +
 * `node:crypto`, loaded only when credential resolution runs.
 */
 ...
 * project access whose `resolveToken` is memoized internally.
 *
