...
| `pyric sandbox fork\|apply\|diff\|promote\|discard\|listBranches` | Work a Firestore change out on a branch before it reaches the live sandbox. A branch holds Firestore documents: a fork copies the live documents and the Realtime Database tree, `--candidateRules` decides verdicts on the branch alone, `diff` compares documents, and `promote` lands documents and installs no rules. A branch is a directory under `.pyric/state/branches/<branch>/`, so it survives a restart: `pyric sandbox fork --branch draft`, `pyric sandbox apply --branch draft --sessionPath .pyric/last-session.json`, `pyric sandbox diff --branch draft`, then `pyric sandbox promote --branch draft --confirm` or `pyric sandbox discard --branch draft`. |
| `pyric sandbox inspect\|events\|seed\|seedFromFixture\|exportFixture\|reset\|checkpoint\|restore\|listCheckpoints` | Read and manage the live sandbox state. A checkpoint is a named on-disk snapshot under `.pyric/state/checkpoints/`: `pyric sandbox checkpoint --name before-cleanup`, then `pyric sandbox restore --name before-cleanup --confirm`. `pyric sandbox events --kind denials` pages the operation log, `pyric sandbox exportFixture --path fixture.json` writes a fixture `seedFromFixture` loads back, and `pyric sandbox reset --scope database --confirm` clears one service. |
