...
> contracts; non-mirrored exports (e.g. `@pyric/cli/serve/worker`,
> `@pyric/cli/verify`, `@pyric/cli/credentials`) are experimental
...
| Subpath | Surface |
|---|---|
| `@pyric/cli/deploy` | `fromServiceAccount`, `getDeploy`, `createFirestoreDeployTools`, `createRtdbDeployTools`, `createHostingDeployTools`, `createFunctionsDeployTools` |
...
| `@pyric/cli/discover` | `crawl`, `findCollectionGroup`, `createRestCrawlerFirestore`, `createFirestoreDiscoverTools` |
| `@pyric/cli/auth` | `getAuthTools` (Identity Toolkit-driven provider + domain configuration) |
