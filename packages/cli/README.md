...
| `pyric mcp` | Start the stdio MCP server and serve ten service tools, one per Firebase capability. Which sandbox it serves depends on what is running: when this project's `.pyric/serve.json` pointer names a running `pyric serve` or `pyric sandbox --bridge`, it attaches and relays to the sandbox that process holds in the browser (a server found only by the port scan is reported, not attached to); otherwise it owns an in-process sandbox, a plain object inside this process that persists to `.pyric/state/in-process.json`, with no browser involved. `--attach` insists on the running one and fails when there is none; `--in-process` insists on owning one and never looks. `--allow-production` enables the `production` methods, which are listed but refused without it; `describe` reports them with `status: 'disabled'` and the sentence that enables it. |
...
| `pyric <tool> <method> [--<arg> <value>...]` | Call one method of the service surface (`firestore`, `database`, `storage`, `auth`, `messaging`, `functions`, `ai_logic`, `rules`, `sandbox`, `assurance`) against this project's sandbox, with the SDK's own method and argument names, e.g. `pyric rules lint --service firestore` or `pyric firestore setDoc --path posts/p1 --data '{"a":1}'`. Any argument reads from a file as `--<arg>-file <path>`, as text for a string argument and parsed JSON for an object one: `pyric rules lint --service firestore --rules-file firestore.rules`. The same records serve `pyric mcp`. Firestore field values are written as JSON in `data`, at any depth: `{"$serverTimestamp": true}`, `{"$increment": <number>}`, `{"$arrayUnion": [...]}`, `{"$arrayRemove": [...]}`, and `{"$deleteField": true}` (accepted by `updateDoc` and a `writeBatch` update, refused elsewhere). The Realtime Database takes Firebase's own `{".sv": "timestamp"}`. Every one of these commands acts on the in-process sandbox in `.pyric/state/in-process.json` and says so on its first line; when a `pyric serve` is running for the project, which holds the app's sandbox in the browser, the command refuses rather than answer about a different sandbox, and `--in-process` overrides that. |
| `pyric auth impersonate\|actAsAdmin\|actAsAnonymous\|useAppSession\|whoami\|sessions` | Set and read the identity your own calls run as, in this project's in-process sandbox. `whoami` reports it beside the app session, the user the sandbox's SDK is signed in as, and says which one the next call runs as. |
...
| `pyric sandbox inspect\|events\|listeners\|activity\|seed\|seedFromFixture\|exportFixture\|reset\|checkpoint\|restore\|listCheckpoints` | Read and manage the live sandbox state. A checkpoint is a named on-disk snapshot under `.pyric/state/checkpoints/`: `pyric sandbox checkpoint --name before-cleanup`, then `pyric sandbox restore --name before-cleanup --confirm`. `pyric sandbox events --kind denials` pages the operation log, `pyric sandbox listeners --service database` lists every attached listener, folded from the attach and detach events, with delivery and suppression counts, `pyric sandbox activity --pattern duplicate-listener` reports the Firestore activity monitor's incidents (`repeated-read`, `duplicate-listener`, `listener-churn`; the Realtime Database is not monitored), `pyric sandbox exportFixture --path fixture.json` writes a fixture `seedFromFixture` loads back, carrying the seeded passwords unless `--excludePasswords` is passed; the file it writes is the seed shape `sandbox seed` accepts (`users` entries of `uid` plus optional `email`, `password`, `customClaims`, and `tenantId`), not the state file `pyric snapshot` writes, and `pyric sandbox reset --scope database --confirm` clears one service. A checkpoint carries the clock, so restoring one restores the instant it was taken under along with the data. |
...
| `pyric messaging send\|subscribeToTopic\|unsubscribeFromTopic\|tokens\|deliveries` | Work Firebase Cloud Messaging in the sandbox. `send` delivers as the FCM server would, its payload nested under `--message`, exactly one of `token`, `topic`, or `condition` naming the recipient: `pyric messaging send --message '{"topic":"news","notification":{"title":"Update"}}'`. `subscribeToTopic` and `unsubscribeFromTopic` take `--tokens '["..."]'` and `--topic`. `tokens` lists every registered device token, its state, and the topics it is subscribed to; `deliveries --since <ms>` lists what the sandbox delivered, foreground or background, handled or not. Both reads change nothing. |
| `pyric functions listTriggers\|fire\|executions` | Work the Cloud Functions RTDB trigger runtime. `listTriggers` discovers the handlers a project's Functions source defines, their reference patterns, and any exports whose trigger kind the runtime does not support yet, with the reason. `fire --trigger makeUppercase --path messages/abc123/original --value '"hello"'` runs a discovered handler on a synthetic event built from `path` and `value`, without writing `value` at `path`. `executions --since <ms>` lists the runs `fire` caused, with cause, duration, and result or error. Callable functions are not mirrored: there is no `call` method. |
| `pyric ai_logic script\|clearScripts\|scripts\|status` | Control AI Logic's local mirror. Never sends a prompt anywhere. `script --match '{"substring":"archive status"}' --response '{"type":"text","payload":"The archive is offline for maintenance."}'` registers one deterministic response, matched by prompt substring, by the model a call names, by both, or by neither (unconditional); `response.payload` is a string for type `text`, an object for type `json`, and `{code, message}` for type `error`, refused naming the accepted form when the two disagree. `clearScripts` empties the queue; `scripts` lists what is queued, each beside whether it has answered a call. Both are refused when the resolved engine is not the scripted one. `status` reports the resolved engine (`scripted`, `openai` for a local loopback, or `gemini` for production pass-through), its model and upstream when it has one, and whether a key is configured, never the key itself. |
...
| `@pyric/cli/vite` | `pyric(opts)`, the dev-only firebase→sandbox swap plugin. Opts: `hosted` (opt-in Node host; SharedWorker remains the default), `rules`, `persist`/`fresh`, `seed`, `capture`, `bridge` (MCP), `ui` (Pyric Studio at `/__pyric/ui/studio`, parity with `dev --ui`). |
...
| `@pyric/cli/remote` | Node-side client for a browser-held sandbox |
