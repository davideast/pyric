...
| `pyric auth impersonate\|actAsAdmin\|actAsAnonymous\|useAppSession\|whoami\|sessions` | Set and read the identity your own calls run as. `whoami` reports it beside the app session, the user the sandbox's SDK is signed in as, and says which one the next call runs as. |
