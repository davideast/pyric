 ...
 * `sdk-service` method that is an enum, at any nesting depth, the method's
 * `signature` line names every value the field can take. An enum a model
 * cannot see before the first call is the failure mode the sdk-service
...
describe('enums appear in the signature line', () => {
  for (const tool of SDK_TOOLS) {
    for (const method of tool.methods) {
      ...
      it(`${tool.name}.${method.name}'s signature names every enum value`, () => {
