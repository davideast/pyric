...
it('serves the embedded docs site (dir index, .md twin, index.json, assets) and 404s misses — never Studio shell', async () => {
    ...
    const page = await fetch(h.url + '/__pyric/ui/docs/cli/');
    ...
    const noSlash = await fetch(h.url + '/__pyric/ui/docs/cli', { redirect: 'manual' });
    expect(noSlash.status).toBe(301);
    expect(noSlash.headers.get('location')).toBe('/__pyric/ui/docs/cli/');

    // The .md agent twin (flat) serves 200.
    const twin = await fetch(h.url + '/__pyric/ui/docs/cli.md');
