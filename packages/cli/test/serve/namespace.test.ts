...
it('serves the embedded docs site (dir index, .md twin, index.json, assets) and 404s misses — never Studio shell', async () => {
    ...
    mkdirSync(join(docsRoot, 'docs', 'cli'), { recursive: true });
    mkdirSync(join(docsRoot, '_astro'), { recursive: true });
    writeFileSync(join(docsRoot, 'docs', 'cli', 'index.html'), '<!doctype html>DOC PAGE');
    writeFileSync(join(docsRoot, 'docs', 'cli.md'), '# raw markdown twin');
    ...
    const page = await fetch(h.url + '/__pyric/ui/docs/@pyric/cli/');
    ...
    const noSlash = await fetch(h.url + '/__pyric/ui/docs/@pyric/cli', { redirect: 'manual' });
    expect(noSlash.status).toBe(301);
    expect(noSlash.headers.get('location')).toBe('/__pyric/ui/docs/@pyric/cli/');

    // The .md agent twin (flat) serves 200.
    const twin = await fetch(h.url + '/__pyric/ui/docs/@pyric/cli.md');
