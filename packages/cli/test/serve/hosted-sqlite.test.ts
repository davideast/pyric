...
test('pyric-admin and the Node remote client move Storage bytes over the byte route, with ranges and streams', async () => {
  expect(await runNodeFixture('node-byte-route', 60_000)).toBe('Node byte route passed');
}, 90_000);
