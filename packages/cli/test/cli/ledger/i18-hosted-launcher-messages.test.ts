...
function runHosted(directory: string, flags: string[] = []): string {
  const result = spawnSync(node, [
    cli, 'sandbox', '--hosted', '--no-open', '--no-watch', '--no-capture',
...
test('hosted fresh explains how to restore its archived directory', () => {
  const directory = seedProject();
  try {
    runHosted(directory, ['--seed', 'seed.json']);
    const fresh = runHosted(directory, ['--fresh']);
