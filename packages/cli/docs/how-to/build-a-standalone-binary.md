...
@pyric/cli' wrapper entries and the `pyric` version baked into @pyric/cli, not
...
`pyric` and `@pyric/cli` are unpublished, so a scaffold that depended on them
...
higher version than the local `0.0.0`**, so `@pyric/cli`' transitive `pyric@*`
would otherwise pull that empty stub instead of the vendored package.
(`@pyric/cli`' own `pyric` dep is rewritten to `*` at pack time so it dedupes to
