...
The handler's `data` field carries the underlying `Outcome` so consumers can re-narrow when they need the structured code. See [Tool factories](./tool-factories.md) for the `DeployToolData` map.
