...
- Working in Node with a service account instead? See [Build a `ProjectScope` from a service account](./build-projectscope-from-service-account.md).
- Want to understand the resolver pattern (why `resolveToken` is a function, not a value)? See [Token caching and `memoizeTtl`](../explanation/token-caching.md).
