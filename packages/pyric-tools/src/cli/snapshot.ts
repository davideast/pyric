 ...
 *
 * Source preference:
 *   1. LIVE state from a running `pyric serve --persist`
 ...
 * `pyric serve --seed <out>` (the state-file shape is detected by its
...
async function fetchLive(port: number): Promise<LiveState | null> {
  try {
    ...
    return null; // nothing listening / not a pyric serve — fall through
  }
}
...
export async function runSnapshot(parsed: ParsedArgs, deps: SnapshotDeps = {}): Promise<number> {
  ...
  for (const port of ports) {
    ...
    if (found.projectDir && found.projectDir !== cwd) {
      ...
      err.write(
        `pyric snapshot: the serve on port ${port} persists a DIFFERENT project\n` +
  ...
  if (!envelope) {
    err.write(
      'pyric snapshot: no state found — no `pyric serve --persist` is running here and ' +
  ...
  report.write(`  Re-serve it: pyric serve --seed ${typeof outFlag === 'string' ? outFlag : 'pyric-state.json'}\n`);
