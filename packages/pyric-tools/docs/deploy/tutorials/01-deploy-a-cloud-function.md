...
The function is reachable at its Cloud Run URL. To route it through Firebase Hosting (so it appears as `/api/hello` on your domain), follow [How to deploy Hosting rewrites](../how-to/deploy-hosting-rewrites.md). To expose the deploy through an agent, see [Register deploy tools with an agent](../how-to/register-tools-with-an-agent.md).
