...
- Don't have a service account? Use [Build a `ProjectScope` from Firebase Auth (browser)](./build-projectscope-from-firebase-auth.md).
- Want to understand why the resolver is memoised at 90% TTL? See [Token caching and `memoizeTtl`](../explanation/token-caching.md).
