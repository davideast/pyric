...
Primitives throw `AdminApiError` on non-2xx; orchestrators return `Outcome`-shaped objects. JSDoc on each function marks which it is. See [Primitives throw, orchestrators return](../explanation/primitives-vs-orchestrators.md).
