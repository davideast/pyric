...
import { hasClaim, hasClaimRole, isMemberOf, hasRole } from 'membership';`,
  ...
  test('enforces receiver types on direct Firestore lookup results and propagated values', () => {
    const modules = [
      `export function broken() {
        return get(/databases/$(database)/documents/users/a).data.matches('x');
      }`,
      `export function broken() {
        return exists(/databases/$(database)/documents/users/a).keys().hasAll([]);
      }`,
      `export function broken() {
        return getAfter(/databases/$(database)/documents/users/a).data.split('x').size() > 0;
      }`,
      `function loaded() {
        return get(/databases/$(database)/documents/users/a);
      }
      export function broken() {
        let document = loaded();
        return document.data.matches('x');
      }`,
      `function misuse(value) {
        return value.keys().hasAll([]);
      }
      export function broken() {
        return misuse(exists(/databases/$(database)/documents/users/a));
      }`,
    ];
