...
describe('lintExpression', () => {
  ...
  test('LOOSE_EQUALITY warning for ==', () => {
    const warnings = lintExpression('auth.uid == "abc"', 'read');
    const codes = warnings.map(w => w.code);
    expect(codes).toContain('LOOSE_EQUALITY');
  });

  test('LOOSE_INEQUALITY warning for !=', () => {
    const warnings = lintExpression('auth !== null != false', 'read');
    // != present somewhere
    const warnings2 = lintExpression('auth.uid != null', 'read');
    expect(warnings2.map(w => w.code)).toContain('LOOSE_INEQUALITY');
  });
