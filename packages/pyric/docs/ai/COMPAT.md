...
| getai-default-backend | With no options the backend defaults to `GoogleAIBackend`, `backendType` is `GOOGLE_AI`, and the AI handle location is the empty string | ✓ | `unit:instances.test.ts` test `ai#getai-default-backend` (matches upstream AIOptions default) |
...
| backend-vertex | `VertexAIBackend` carries `backendType` `VERTEX_AI`; its location and the resulting AI handle location default to `us-central1` | ✓ | `unit:instances.test.ts` test `ai#backend-vertex` (matches upstream constructor default) |
