...
For coverage against `firebase/auth`'s full surface, see [feature-matrix.md](./feature-matrix.md).
...
Does NOT gate the `sandbox.setUser` test driver — that bypass has no prod analog (it already skips provider enforcement the same way). Over the served-worker path (`@pyric/cli`'s SharedWorker-backed auth), registering throws immediately rather than silently accepting a callback that could never run — see COMPAT.md.
...
These back the `sandbox.*` methods below; see [sandbox-test-driver.md](./sandbox-test-driver.md) for usage.
