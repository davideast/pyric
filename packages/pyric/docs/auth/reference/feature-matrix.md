...
| `onIdTokenChanged(auth, observer)` | ✅ | Sandbox: fires on identity change AND on `getIdToken(true)` forced refresh — matches prod (oracle `auth-onidtokenchanged-force-refresh.json`). Does not fire *spontaneously* (sandbox tokens don't expire). | Yes |
...
See [sandbox-test-driver.md](./sandbox-test-driver.md) for the full driver API.
