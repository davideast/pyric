...
The Firebase Emulator Suite is bit-for-bit production. Same Firestore engine, same rules engine, same wire protocols. Tests written against the emulator and tests written against production differ only in their connection URL. If you have the ability to run it — a developer laptop, a CI runner with Java installed, a container — it's a perfectly good choice.
...
See [Why service adapters live in sibling packages](./why-adapters-are-siblings.md) for the longer version.
