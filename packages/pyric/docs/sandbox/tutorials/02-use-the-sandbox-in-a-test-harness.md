...
This tutorial assumes you completed [Your first sandbox session](./01-your-first-sandbox-session.md) and you have `pyric/sandbox` and `pyric-admin` installed. It uses Bun's test runner; the structure is identical with Vitest or Jest.
...
- For more on the test patterns, see [Reset between tests](../how-to/reset-between-tests.md).
- For when admin reads are necessary, see [Use admin reads to assert in tests](../how-to/use-admin-reads.md).
- For multi-tenant test setups, see [Run multiple isolated sandboxes in parallel](../how-to/multiple-isolated-sandboxes.md).
