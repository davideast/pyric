...
Turn it off when you *want* to test "does the captured stream still pass under today's clock?" — useful for catching rules that depend on a time that's now in the past.
...
- [Observe sandbox events — how-to](./observe-events.md) — the live channel (`onEvent`) that feeds into `history()`.
