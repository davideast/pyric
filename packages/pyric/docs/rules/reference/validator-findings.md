...
Read rules don't carry a `request.resource`. The field is unset, so the predicate silently denies.
...
Same shape as the linter's `PERMISSIVE_RULE`. Reported here at lower severity than the linter blocks the deploy at, because the validator is advisory.
