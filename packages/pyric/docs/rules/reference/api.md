...
See [Errors](./errors.md) for the full picture, including the internal-engine error types these are built from.
...
See [Value wrappers](./value-wrappers.md) for the underlying classes these helpers construct.
...
See [`pyric/database`'s constraints reference](../../database/reference/constraints.md) for the DSL itself.
...
- `pyric/rules/internal/rtdb`: the pure RTDB engine (`RtdbMapper`, `SimulateHandler`, replay, grammar helpers, `RtdbIR`, `RtdbNode`, the constraints DSL, and related types).
...
See [AST reference](./ast.md), [Errors](./errors.md), [Lint rules](./lint-rules.md), [Validator findings](./validator-findings.md), [Stdlib modules](./stdlib-modules.md), and [Simulator context](./simulator-context.md) for the engine detail behind these seams.
