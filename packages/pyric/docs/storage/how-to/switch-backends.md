...
- **`rules` option** at config time. Prod deploys rules separately with `firebase deploy --only storage:rules`.
...
Note that not every `firebase/storage` symbol is re-exported. See [Implementation scope and deferred features](../explanation/implementation-scope.md) for what's missing.

## Where to look next

- For the data and rules wiring, see [Enforce Storage rules](./enforce-rules.md).
- For why the prod entry has fewer options, see [`StorageOptions`](../reference/storage-options.md).
