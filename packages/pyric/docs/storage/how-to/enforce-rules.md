...
Useful when iterating on rule logic. See [Test rule expressions independently](./test-rule-expressions.md).

## Where to look next

- For the supported rule grammar, see [Storage rules subset](../reference/rules-subset.md).
- For the carve-out and other production patterns, see the [Implementation scope and deferred features](../explanation/implementation-scope.md) page.
