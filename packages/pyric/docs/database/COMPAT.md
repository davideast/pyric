...
| M77 | `goOffline(db)` is an accepted no-op on sandbox targets: there is no network connection in the local sandbox to toggle — every op is already a direct, local call. Forwards to `firebase/database`'s real `goOffline` on prod targets | ⚠ no network connection in the local sandbox to toggle | `unit:modular/connection-noops.test.ts` ("goOffline(db) does not throw and is accepted on a sandbox handle") |
| M78 | `goOnline(db)` is an accepted no-op on sandbox targets, symmetric with `goOffline` — since the sandbox was never taken offline, there is nothing to bring back online. Forwards to `firebase/database`'s real `goOnline` on prod targets | ⚠ no network connection in the local sandbox to toggle | `unit:modular/connection-noops.test.ts` ("goOnline(db) does not throw and is accepted on a sandbox handle") |
| M79 | `forceLongPolling()` is an accepted no-op — transport selection (WebSocket vs long-polling) is not applicable to the in-process/worker sandbox, which never opens a wire transport of any kind | ⚠ transport selection not applicable to the in-process/worker sandbox | `unit:modular/connection-noops.test.ts` ("forceLongPolling() is an accepted no-op") |
| M80 | `forceWebSockets()` is an accepted no-op, symmetric with `forceLongPolling` — transport selection not applicable to the in-process/worker sandbox | ⚠ transport selection not applicable to the in-process/worker sandbox | `unit:modular/connection-noops.test.ts` ("forceWebSockets() is an accepted no-op") |
| M81 | `enableLogging(logger?, persistent?)` accepts the real SDK's signature and does nothing — the sandbox has no wire protocol running underneath it to log diagnostics for, so there is nothing to wire the logger into | ⚠ no wire protocol in the sandbox to log diagnostics for | `unit:modular/connection-noops.test.ts` ("enableLogging(logger, persistent) accepts a logger callback + persistent flag") |
