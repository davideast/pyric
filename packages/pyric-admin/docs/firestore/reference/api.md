...
**Errors.** Operations, and every object they return, re-throw failures as `SandboxError` with structured `denialContext` on permission denials. See [error translation](../explanation/error-translation.md).
...
Web-SDK-shaped streaming reads. Four overload groups per target kind mirror `firebase/firestore`'s `onSnapshot`, so call sites copied from production typecheck unchanged. Full overload list and rationale: [`onSnapshot` overloads](./onsnapshot.md).
...
Detailed per-method behavior: [`SandboxFirestore` surface](./sandbox-firestore.md), [How to write a batch](../how-to/write-a-batch.md), [How to run a transaction](../how-to/run-a-transaction.md).
...
`setRules` swaps the active ruleset (parse errors leave the old rules in place), `seed` replaces stored documents while preserving rules, `snapshot` captures every document as a path-keyed map. Full contract: [`SandboxFirestore` surface](./sandbox-firestore.md).
...
- [`SandboxFirestore` surface](./sandbox-firestore.md) for per-method behavior on the handle.
- [`onSnapshot` overloads](./onsnapshot.md) for the streaming surface in full.
