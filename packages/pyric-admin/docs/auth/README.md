...
One honest note up front: sandbox tokens are deterministic strings, not real JWTs. They round-trip through the same sandbox backend and nothing else. The [API reference](./reference/api.md) documents the exact format.

## Where to go next

- [API reference](./reference/api.md) for every method, per arm, including the full not-implemented list.
