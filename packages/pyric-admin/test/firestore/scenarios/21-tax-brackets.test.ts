 ...
 * skipped here because it asserts on `r.data` which the sandbox `runOp`
 * helper does not expose. Original assertion: `expect(r.data).toBeDefined()`.
 */
...
describe('Scenario 21: US Federal Tax Bracket Verification', () => {
  ...
  test('1. correct standard deduction for single filer', async () => {
    const root = makeRoot(RULES, SEED);
    const data = computeTax(85000);
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r1', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(true);
  });

  test('2. wrong standard deduction rejected', async () => {
    const root = makeRoot(RULES, SEED);
    const data = { ...computeTax(85000), standardDeduction: 15000, taxableIncome: 70000 };
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r2', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(false);
  });

  test('3. wrong taxable income rejected', async () => {
    const root = makeRoot(RULES, SEED);
    const data = { ...computeTax(85000), taxableIncome: 70000 };
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r3', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(false);
  });

  // ═══ Category 2: Single Bracket Income (3 tests) ═══

  test('4. income in bracket 1 only ($20k gross)', async () => {
    const root = makeRoot(RULES, SEED);
    const data = computeTax(20000);
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r4', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(true);
  });

  test('5. taxable income exactly at bracket 1 boundary', async () => {
    const root = makeRoot(RULES, SEED);
    const data = computeTax(26200);
    expect(data.taxableIncome).toBe(11600);
    expect(data.b1Tax).toBe(1160);
    expect(data.b2Tax).toBe(0);
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r5', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(true);
  });

  test('6. taxable income is zero (gross equals deduction)', async () => {
    const root = makeRoot(RULES, SEED);
    const data = computeTax(14600);
    expect(data.taxableIncome).toBe(0);
    expect(data.totalTax).toBe(0);
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r6', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(true);
  });

  // ═══ Category 3: Multi-Bracket Income (4 tests) ═══

  test('7. income straddles brackets 1-2 ($30k taxable)', async () => {
    const root = makeRoot(RULES, SEED);
    const data = computeTax(44600);
    expect(data.taxableIncome).toBe(30000);
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r7', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(true);
  });

  test('8. income straddles brackets 1-3 ($70,400 taxable)', async () => {
    const root = makeRoot(RULES, SEED);
    const data = computeTax(85000);
    expect(data.taxableIncome).toBe(70400);
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r8', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(true);
  });

  test('9. income straddles brackets 1-5 ($200k taxable)', async () => {
    const root = makeRoot(RULES, SEED);
    const data = computeTax(214600);
    expect(data.taxableIncome).toBe(200000);
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r9', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(true);
  });

  test('10. top bracket income ($1M taxable)', async () => {
    const root = makeRoot(RULES, SEED);
    const data = computeTax(1014600);
    expect(data.taxableIncome).toBe(1000000);
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r10', auth: { uid: 'alice' }, data });
    // RULES-B5: prod computes bracket taxes with INT64 division (truncates);
    // computeTax carries sub-cent floats → mismatch → DENY. See
    // design rationale
    expect(r.allowed).toBe(false);
  });
  ...
  test('11. income at bracket 2 max boundary ($47,150)', async () => {
    const root = makeRoot(RULES, SEED);
    const data = computeTax(47150 + 14600);
    expect(data.taxableIncome).toBe(47150);
    expect(data.b3Tax).toBe(0);
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r11', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(true);
  });

  test('12. one dollar into bracket 3 ($47,151)', async () => {
    const root = makeRoot(RULES, SEED);
    const data = computeTax(47151 + 14600);
    expect(data.taxableIncome).toBe(47151);
    expect(data.b3Tax).toBe(0.22);
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r12', auth: { uid: 'alice' }, data });
    // RULES-B5: rules-side `1 * 22 / 100` int-divides to 0; `0.22 == 0` is
    // false → prod DENIES (see step-14 doc).
    expect(r.allowed).toBe(false);
  });

  test('13. income at bracket 6 max ($609,350)', async () => {
    const root = makeRoot(RULES, SEED);
    const data = computeTax(609350 + 14600);
    expect(data.taxableIncome).toBe(609350);
    expect(data.b7Tax).toBe(0);
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r13', auth: { uid: 'alice' }, data });
    // RULES-B5: bracket 6's tax is fractional (.75) at this income — int
    // division truncates rules-side → mismatch → prod DENIES (step-14 doc).
    expect(r.allowed).toBe(false);
  });

  test('14. one dollar into bracket 7 ($609,351)', async () => {
    const root = makeRoot(RULES, SEED);
    const data = computeTax(609351 + 14600);
    expect(data.taxableIncome).toBe(609351);
    expect(data.b7Tax).toBe(0.37);
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r14', auth: { uid: 'alice' }, data });
    // RULES-B5: rules-side `1 * 37 / 100` int-divides to 0; `0.37 == 0` is
    // false → prod DENIES (see step-14 doc).
    expect(r.allowed).toBe(false);
  });
  ...
  test('15. wrong b1Tax rejected', async () => {
    const root = makeRoot(RULES, SEED);
    const data = { ...computeTax(85000), b1Tax: 1100 };
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r15', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(false);
  });

  test('16. wrong b2Tax rejected', async () => {
    const root = makeRoot(RULES, SEED);
    const data = { ...computeTax(85000), b2Tax: 4200 };
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r16', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(false);
  });

  test('17. wrong b3Tax rejected', async () => {
    const root = makeRoot(RULES, SEED);
    const data = { ...computeTax(85000), b3Tax: 5000 };
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r17', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(false);
  });

  test('18. wrong b4Tax rejected', async () => {
    const root = makeRoot(RULES, SEED);
    const data = { ...computeTax(200000), b4Tax: 999 };
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r18', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(false);
  });

  test('19. wrong b5Tax rejected', async () => {
    const root = makeRoot(RULES, SEED);
    const data = { ...computeTax(260000), b5Tax: 999 };
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r19', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(false);
  });

  test('20. wrong b7Tax rejected (top bracket)', async () => {
    const root = makeRoot(RULES, SEED);
    const data = { ...computeTax(1014600), b7Tax: 999 };
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r20', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(false);
  });

  test('21. wrong totalTax (bracket taxes correct, sum wrong)', async () => {
    const root = makeRoot(RULES, SEED);
    const data = { ...computeTax(85000), totalTax: 99999 };
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r21', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(false);
  });

  // ═══ Category 6: Auth Enforcement (3 tests) ═══

  test('22. unauthenticated user rejected', async () => {
    const root = makeRoot(RULES, SEED);
    const data = computeTax(85000);
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r22', auth: null, data });
    expect(r.allowed).toBe(false);
  });

  test('23. wrong userId rejected', async () => {
    const root = makeRoot(RULES, SEED);
    const data = computeTax(85000, 'bob');
    const r = await runOp(root, { method: 'create', path: 'tax_returns/r23', auth: { uid: 'alice' }, data });
    expect(r.allowed).toBe(false);
  });

  // Test #24 ('owner can read own return') skipped — requires r.data which runOp does not expose

  // ═══ Category 7: Config & Immutability (4 tests) ═══

  test('25. config doc cannot be written', async () => {
    const root = makeRoot(RULES, SEED);
    const r = await runOp(root, { method: 'create', path: 'tax_config/2025', auth: { uid: 'admin' }, data: { b1Max: 12000 } });
    expect(r.allowed).toBe(false);
  });

  test('26. filed return cannot be updated', async () => {
    const root = makeRoot(RULES, SEED);
    const r = await runOp(root, { method: 'update', path: 'tax_returns/existing1', auth: { uid: 'alice' }, data: { totalTax: 0 } });
    expect(r.allowed).toBe(false);
  });

  test('27. filed return cannot be deleted', async () => {
    const root = makeRoot(RULES, SEED);
    const r = await runOp(root, { method: 'delete', path: 'tax_returns/existing1', auth: { uid: 'alice' } });
    expect(r.allowed).toBe(false);
  });

  test('28. non-owner cannot read return', async () => {
    const root = makeRoot(RULES, SEED);
    const r = await runOp(root, { method: 'get', path: 'tax_returns/existing1', auth: { uid: 'bob' } });
    expect(r.allowed).toBe(false);
  });
});
