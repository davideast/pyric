...
describe('Scenario 9: Rook + King Chess Subset', () => {
  test('king adjacent move', async () => {
    const root = makeRoot(RULES, makeSeed());
    const r = await runOp(root, { method: 'update', path: 'games/g1', auth: { uid: 'white' }, data: game({
      a1: '', b1: 'K', hp_K: 'b1',
  ...
  test('rook vertical slide', async () => {
  ...
  test('rook horizontal slide', async () => {
    ...
    const root2 = makeRoot(RULES, makeSeed({ a1: '', b2: 'K', hp_K: 'b2' }));
    const r2 = await runOp(root2, { method: 'update', path: 'games/g1', auth: { uid: 'white' }, data: game({
      b2: 'K', hp_K: 'b2',
      d1: '', b1: 'R', hp_R: 'b1',
      a1: '',
      moveFrom: 'd1', moveTo: 'b1', moveCount: 1, currentTurn: 'guest',
  ...
  test('king into rook check denied', async () => {
    ...
    const root = makeRoot(RULES, makeSeed({ d4: '', a4: 'r', gp_r: 'a4', d3: 'k', gp_k: 'd3' }));
    const r = await runOp(root, { method: 'update', path: 'games/g1', auth: { uid: 'white' }, data: game({
      a1: '', a2: 'K', hp_K: 'a2',
  ...
  test('rook diagonal denied (invalid geometry)', async () => {
    const root = makeRoot(RULES, makeSeed());
    const r = await runOp(root, { method: 'update', path: 'games/g1', auth: { uid: 'white' }, data: game({
      d1: '', c2: 'R', hp_R: 'c2',
  ...
  test('wrong player denied', async () => {
    const root = makeRoot(RULES, makeSeed());
    const r = await runOp(root, { method: 'update', path: 'games/g1', auth: { uid: 'black' }, data: game({
      a4: '', a3: 'k', gp_k: 'a3',
  ...
  test('king teleport denied', async () => {
  ...
  test('unauthenticated denied', async () => {
    const root = makeRoot(RULES, makeSeed());
    const r = await runOp(root, { method: 'update', path: 'games/g1', auth: null, data: game({
      a1: '', b1: 'K', hp_K: 'b1',
      moveFrom: 'a1', moveTo: 'b1', moveCount: 1, currentTurn: 'guest',
    }) });
    expect(r.allowed).toBe(false);
