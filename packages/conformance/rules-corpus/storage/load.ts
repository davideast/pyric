 ...
 * `rules-corpus/storage/` is the index: one authored `StorageScenarioRecord` per
 * file, named `<scenario-id>.ts`. This loader reads the directory, requires
 * every scenario file (skipping `types.ts`, `load.ts`, and `index.ts`), derives
 * each scenario's id from its filename, validates the record, and returns the
 ...
 * (`ALL_RULES_STORAGE_SCENARIOS`) read a plain array at module-evaluation time.
 *
...
import type { StorageScenario, StorageScenarioRecord } from './types.ts';
...
/** Loads every Storage rules scenario in this directory, validating each record
 *  and injecting its id from the filename. Throws with every problem found
 *  rather than silently dropping a bad file. */
export function loadStorageScenarioRecords(): StorageScenario[] {
  ...
  for (const file of files) {
    ...
    const mod = require(join(HERE, file)) as { scenario?: StorageScenarioRecord; default?: StorageScenarioRecord };
    ...
    scenarios.push({ id, ...(record as StorageScenarioRecord) });
  }
...
/** The loaded scenarios, evaluated once — the `StorageScenario[]` shape every
 *  consumer (capture runner, replay suite) has always seen. */
export const loadedStorageScenarios: StorageScenario[] = loadStorageScenarioRecords();
