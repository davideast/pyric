 ...
 * allow/deny, then restoring the prior ruleset. Every pack here is a
 ...
 *
 * An `RtdbPack` is a self-contained conformance unit: one ruleset SUBTREE plus
 * the ops that exercise it. `rules` is the JSON string of the subtree exactly as
 * the agreement probe deployed it; the subtree mounts under the pack id (the
 ...
 * the subtree under the pack id and run the same ops, so a pack hands off to
 ...
 *  production verdict frozen for that op. `opPath` is relative to the pack's
...
export interface RtdbTestCase {
  /** Unique within the pack; the verdict-table key in observations. Carried
  ...
  /** Path relative to the pack's mount key. May contain the `<UID>` token. */
  opPath: string;
  authPresent: boolean;
...
/**
 * An RTDB rules conformance pack. Same provenance fields as the Firestore
 * {@link import('../firestore/types.ts').Pack} (so the corpora stay legible
 * side by side), plus `provenance` — the frozen seed observation this pack was
 * decomposed from — and RTDB-shaped cases.
 */
export interface RtdbPack {
  ...
  rationale: string;
  /** The seed evidence this pack was decomposed from — cites the frozen
   ...
   *  probe deployed it. Mounts under the pack id at capture/replay time. */
  rules: string;
 ...
 * pack id, so the record carries no `id` field — the loader (./load.ts) injects
 * it from the filename.
 */
export type RtdbPackRecord = Omit<RtdbPack, 'id'>;
