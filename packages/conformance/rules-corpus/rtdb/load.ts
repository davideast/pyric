 ...
 * `RtdbPackRecord` per file, named `<pack-id>.ts`. This loader reads the
 * directory, requires every pack file (skipping `types.ts`, `load.ts`, and
 * `index.ts`), derives each pack's id from its filename, validates the record,
 * and returns the typed array sorted by id.
 *
 * Loading is synchronous (Bun's `require` handles `.ts`), for the same reason
 * as the sibling loaders: existing consumers (`ALL_RULES_RTDB_PACKS`) read a
 * plain array at module-evaluation time.
 *
 * Validation is CI-enforced input, not best-effort: a malformed pack is a hard
 ...
 * non-empty rules), RTDB packs additionally require a non-empty `provenance`
...
import type { RtdbPack, RtdbPackRecord } from './types.ts';
...
function recordProblems(file: string, id: string, value: unknown): string[] {
  ...
  if (typeof record.provenance !== 'string' || !record.provenance.trim()) {
    fail("missing 'provenance' — RTDB packs must cite the frozen seed observation they were decomposed from");
  }
...
/** Loads every RTDB rules pack in this directory, validating each record and
 *  injecting its id from the filename. Throws with every problem found rather
 *  than silently dropping a bad file. */
export function loadRtdbPackRecords(): RtdbPack[] {
  const files = readdirSync(HERE)
    .filter((file) => file.endsWith('.ts') && !NON_RECORD_FILES.has(file))
    .sort();

  const problems: string[] = [];
  const packs: RtdbPack[] = [];

  for (const file of files) {
    const id = file.slice(0, -'.ts'.length);
    const mod = require(join(HERE, file)) as { pack?: RtdbPackRecord; default?: RtdbPackRecord };
    ...
    packs.push({ id, ...(record as RtdbPackRecord) });
  }
...
/** The loaded packs, evaluated once — the `RtdbPack[]` shape every consumer
 *  (capture runner, replay suite) has always seen. */
export const loadedRtdbPacks: RtdbPack[] = loadRtdbPackRecords();
