    ...
    },
    {
      ...
      rows: [
        ...
        },
        {
          ...
          "section": "`sandbox.*` (sandbox-only test driver)",
          "api": "sandbox.*` (sandbox-only test driver)",
          "behavior": "All `sandbox.*` methods operate only on the sandbox backend attached to a mirror-produced `Auth` handle; there is no production target or production branch",
          "status": "conforms",
          "evidence": "`unit:sandbox-test-driver.test.ts`, `unit:sandbox-user-admin.test.ts` + compiled client-binding isolation",
          "risk": ["structural"],
          "riskScore": 0,
          "riskReasons": ["structural / routing-only claim"],
          "automation": "unit-backed",
