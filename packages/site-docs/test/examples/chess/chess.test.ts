...
import { createEmptyChessGame, type ChessGame } from '../../../src/examples/chess/run';
...
const SCENARIOS: TestCase[] = [
  ...
  scenario('checkmate claim while the king is in check', 'ALLOW',
    game({ e1: 'K', hp_K: 'e1', h5: 'Q', hp_Q: 'h5', e8: 'k', gp_k: 'e8' }),
    game({ e1: 'K', hp_K: 'e1', h5: 'Q', hp_Q: 'h5', e8: 'k', gp_k: 'e8', status: 'won', moveType: 'checkmate' })),
  scenario('false checkmate claim', 'DENY',
    game({ e1: 'K', hp_K: 'e1', a1: 'Q', hp_Q: 'a1', e8: 'k', gp_k: 'e8' }),
    game({ e1: 'K', hp_K: 'e1', a1: 'Q', hp_Q: 'a1', e8: 'k', gp_k: 'e8', status: 'won', moveType: 'checkmate' })),
...
describe('chess v2 production scenarios', () => {
  ...
  test('ports the production-observed v2 rules artifact', () => {
    const digest = createHash('sha256').update(RULES.trimEnd()).digest('hex');
    expect(digest).toBe('45aea8a5ef6548dbfc56392214c4d86867543c840f52f281b92359fc5956937f');
  });
