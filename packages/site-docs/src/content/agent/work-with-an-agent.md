...
Each MCP tool covers one service and artifact, such as `firestore_data` or `firestore_rules`, and takes a required `op` field that selects the operation. This page writes a call as `tool.op`; the agent calls the tool and sets `op`.
...
The agent starts with `sandbox.inspect`, which returns the loaded rules, lint summary, document counts, and recent requests. It can then use `firestore_simulator.events` to inspect the request history and `firestore_data.get` to compare the stored document with the attempted write.
...
The agent can create the document with `firestore_data.set`, read it with `firestore_data.get`, and run the two hypothetical requests with `firestore_rules.simulate`. For a longer sequence it can open a stateful session with `firestore_simulator.create`, execute operations with `firestore_simulator.execute`, and inspect the result with `firestore_simulator.read`.
...
That task exercises `firestore_data.set` and `firestore_data.query` against the same data visible in the browser.
...
- `firestore_rules.lint` to catch invalid rules syntax, production limits, and common JavaScript-shaped mistakes;
- `firestore_rules.simulate` to evaluate explicit allow and deny cases;
- `rules_stdlib.list` and `rules_stdlib.get`, with `service` set to `firestore`, to find tested helpers;
- `firestore_rules.resolve` to compile `2+modules` imports into deployable Firestore Rules.
...
The agent uses `database_data.crawl` to discover the current paths and `database_rules.simulate` to test the request. These tools inspect and simulate local state; they do not connect to a production database.
