...
<span class="compat-stat-pct">69.1%</span>
<span class="compat-stat-label">of public runtime exports supported</span>
</p>
<p class="compat-stat-denom">38 of 55 public runtime exports <span aria-hidden="true">·</span> 109 of 164 public type exports</p>
</div>
...
|---|---|---|---|---|
| ai.imagen-retirement | out-of-scope | `ImagenAspectRatio`, `ImagenImageFormat`, `ImagenModel`, `ImagenPersonFilterLevel`, `ImagenSafetyFilterLevel`, `getImagenModel` | Imagen is deprecated upstream; all Imagen models shut down as early as June 2026 (upstream 2.11.0 deprecation). Mirroring an API whose production counterpart is retiring would freeze dead behavior. | upstream:firebase/ai@2.11.0 |
| ai.hybrid-inference | deferred | `InferenceMode` | Hybrid/on-device inference is deferred, not out of scope — the sandbox runs in the browser and can model the on-device path through the answer-engine seam; the mode has not been wired yet. | upstream:firebase/ai |
| ai.inference-source | deferred | `InferenceSource` | Deferred with hybrid/on-device inference — the source marker is meaningful once the hybrid mode lands. | disposition:ai.hybrid-inference |
| ai.live-api | deferred | `LiveGenerativeModel`, `LiveResponseType`, `LiveSession`, `getLiveGenerativeModel` | Live API is deferred, not out of scope — it is a bidirectional websocket protocol the sandbox can model with a scripted session engine, the same seam pattern the REST plane already uses; the work has not happened yet. | upstream:firebase/ai |
| ai.server-templates | deferred | `TemplateGenerativeModel`, `getTemplateGenerativeModel` | Server-side templates are deferred, not out of scope — hosting template state is exactly the kind of server infrastructure the sandbox exists to model; the template store has not been built yet. | upstream:firebase/ai |
| ai.template-imagen-retirement | out-of-scope | `TemplateImagenModel`, `getTemplateImagenModel` | Template-served Imagen models retire with the rest of Imagen (deprecated upstream, June 2026 shutdown); the template transport does not outlive the model family it serves. | upstream:firebase/ai@2.11.0 |
| ai.live-audio | deferred | `startAudioConversation` | Deferred with the rest of the Live API family — the browser audio helper rides the Live session seam once that lands. | disposition:ai.live-api |
