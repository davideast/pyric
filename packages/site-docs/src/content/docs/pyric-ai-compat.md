...
| Metric | Value |
|---|---|
| SDK surface coverage | 69.1% of the total surface, 80.9% of the intended surface |
| Behavior conformance | 74 of 80 evaluated rows conform (6 diverged-documented) |
| Live-prod oracle citations | 39 of 80 rows |

Generated by `bun run compat:generate`. Full rollup: [Conformance scores by surface](../pyric-conformance-scores/).

This matrix is the row-by-row contract of what AI Logic guarantees against the production Firebase surface, generated from the conformance registry.
