...
1. **String key**: `Firestore.target`. Discoverable through `Object.keys`, serialisable, easy for consumers to read. Becomes API surface.
2. **Module-private state map**: a `WeakMap<Firestore, Target>` inside the package. Truly hidden, but consumers can't even reference the handle's shape — type checks lose information.
...
If we exposed a property on our `Firestore`, application code might start to depend on it. That code would then break the moment the project actually migrated to `firebase/firestore` (or vice versa). Opacity is what keeps the migration boundary clean.
