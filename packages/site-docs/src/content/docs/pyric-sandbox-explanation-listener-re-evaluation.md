...
- **Reproducing a production bug** where the symptom is "a listener kept its old gating". The sandbox can't reproduce this — every rule change re-gates everything. You'd need the emulator or live Firestore.
