...
> **These scores are generated, not asserted.** Regenerated by `bun run compat:generate` from three artifacts: `packages/conformance/baselines/coverage-baseline.json` (SDK surface breadth and behavior row statuses), `packages/conformance/rules-language/coverage-report.json` (rules-language construct coverage), and `packages/conformance/assurance-capabilities/capabilities.json` (assurance capabilities). Editing this file by hand does nothing; the numbers come from the artifacts.

Each per-surface COMPAT matrix shows the status of every row. This page sums those rows into one number per dimension per surface, so the aggregate is readable in one place. The COMPAT matrix is the receipt behind each cell.

Across every measured SDK surface, breadth is 53.5% of the total surface and 62.9% of the intended surface.
