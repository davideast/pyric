...
> **Experimental.** Storage works and is documented, but most of its behavior is not yet pinned to a recorded production observation the way Auth and Firestore are. Read [the conformance scores](../pyric-conformance-scores/) before you rely on it.
...
If the data is structured rather than binary, [which data service should I use?](../which-data-service/) sorts it out. And [the conformance scores](../pyric-conformance-scores/) show exactly what experimental costs you.
