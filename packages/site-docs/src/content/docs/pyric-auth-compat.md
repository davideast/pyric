...
<details class="compat-row" data-status="unsupported">
<summary class="compat-line"><span class="compat-num">175</span><span class="compat-dot" data-status="unsupported" role="img" aria-label="Unsupported" title="Unsupported"></span><span class="compat-behavior">Not implemented. Deprecated upstream: production returns an empty list when email-enumeration protection is on (the default), so mirroring it would mislead callers.</span></summary>
...
| 175 | Not implemented. Deprecated upstream: production returns an empty list when email-enumeration protection is on (the default), so mirroring it would mislead callers. |
