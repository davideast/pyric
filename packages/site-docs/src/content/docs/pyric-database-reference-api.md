...
- **The whole service is experimental.** Sandbox behavior is verified by unit probes; only the semantics with an oracle citation are pinned to recorded production behavior. See the [conformance matrix](../pyric-database-compat/) for the row-by-row state.
...
For the rules constraint DSL and deploy workflow, see [rules-tooling.md](../pyric-database-reference-rules-tooling/). For coverage against `firebase/database`'s full surface, see the [conformance matrix](../pyric-database-compat/).
