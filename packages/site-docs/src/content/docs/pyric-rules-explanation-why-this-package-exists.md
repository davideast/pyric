...
`pyric/firestore` is meant to be a drop-in replacement for `firebase/firestore`. Any host that imports it should see exactly the surface area that the upstream modular Web SDK exposes. No more, no less. Adding `lintFirestoreRules` to that import path would either pollute the swap-in surface (making the package look bigger than the production package it mirrors) or force consumers to import from non-standard sub-paths (which break tree-shaking and make TypeScript editor completion noisier than it needs to be).

By the time you have `lintFirestoreRules`, `SimulateFirestoreRulesHandler`, `parseToAST`, `validateFirestoreRules`, plus seven value wrappers and a sentinel expression engine, the rules tooling alone is bigger than the production data-plane surface. Bundling them would dominate the swap-in package.
