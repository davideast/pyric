...
Pyric's agent runtime produces sessions, each a record of one task. The session captures the prompt, the tool calls, the LLM responses, the events streamed to the UI, the final outcome. A session is small (kilobytes, occasionally a megabyte) but unbounded: there can be thousands of them over time.
