...
The current package is the result. Where the CLI does five REST calls to deploy a Hosting site, `hosting.deployFiles` makes the same five calls. The wire shapes are the same, the operations are the same — just exposed as TypeScript functions instead of CLI subcommands.

## What we lose

A few things the CLI does that this package doesn't:

- **Interactive confirmation.** A CLI deploy can prompt before destructive operations. Our primitives just do what they're told. Callers that want confirmation build their own UI on top.
...
- **Cross-product flows.** The CLI's `firebase deploy` does Hosting + Functions + Firestore in one verb. Our package exposes them separately — callers compose them.
