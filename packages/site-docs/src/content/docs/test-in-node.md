...
Your captured dev sessions double as a suite. `pyric verify journeys/` replays every fixture in the directory against candidate rules and exits non-zero on a real divergence, which slots straight into CI next to your unit tests. See [Ship to production](../ship-to-production/).

## Where to go next

The seed and reset patterns these tests lean on are covered in [Shape your data](../shape-your-data/). To watch a failing test's traffic instead of guessing, see [See what's happening](../see-whats-happening/).
