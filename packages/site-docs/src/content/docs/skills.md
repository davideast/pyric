...
Realtime Database rules cascade downward, so a permissive parent grants every descendant and a child cannot take it back. Realtime Database support in Pyric is experimental; [the conformance scores](../pyric-conformance-scores/) show what that costs you.
...
One JSON tree, where reading a path downloads everything below it, so structure is performance. Realtime Database support in Pyric is experimental; [the conformance scores](../pyric-conformance-scores/) show what that costs you.
