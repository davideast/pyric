...
| Metric | Value |
|---|---|
| SDK surface coverage | 100% of the total surface, 100% of the intended surface (client receive plane; the admin send plane has no separate breadth entry) |
| Behavior conformance | 56 of 56 evaluated rows conform |
| Live-prod oracle citations | 15 of 56 rows |

Generated by `bun run compat:generate`. Full rollup: [Conformance scores by surface](../pyric-conformance-scores/).

This matrix is the row-by-row contract of what Messaging guarantees against the production Firebase surface, generated from the conformance registry.
