...
| `disabledReason` | `string` | Why — stamped on `data-disabled-reason` (only while `disabled`, alongside `aria-disabled`). Canonical source is the rules gate: `disabled={!gate.verdictFor(path).upload}` + `disabledReason={verdict.reasons.write.join('; ')}` — see [rules-aware affordances](./rules-aware-affordances.md). |
...
- [`<ObjectBrowser>`](./ObjectBrowser.md) — typically rendered inside the
