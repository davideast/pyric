...
`PYRIC_GUARD` selects what the network guard does when a Pyric-launched process opens a connection to a live Google or Firebase endpoint. A sandboxed app routes all of its Firebase traffic to the local `/__pyric/*` namespace, so such a connection means that process escaped the sandbox and is reading or writing production data. The value is `block` (the default: report the connection on stderr and fail the request), `warn` (report it, let it through), or `off` (install no hooks at all and print one notice saying so). An unset, empty, or unrecognized value means `block`. Each refusal names the two ways to let that host through: add it to `PYRIC_GUARD_ALLOW`, or set `PYRIC_GUARD=warn`. The GCE metadata server is refused under `warn` and `block` in every case, because a development process has no legitimate reason to fetch credentials from it.
