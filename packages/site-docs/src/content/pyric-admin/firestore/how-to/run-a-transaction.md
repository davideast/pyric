...
For multi-doc writes that don't need read-then-write semantics, use a `WriteBatch` instead. See [Write a batch](./write-a-batch.md).
...
- For the substrate's read-tracking behaviour, see [`pyric/sandbox`'s internal protocol](../../../sandbox/docs/reference/internal-protocol.md).
