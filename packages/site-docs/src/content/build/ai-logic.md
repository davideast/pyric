...
`pyric sandbox` runs your command under a network guard that refuses connections to live Google endpoints. It permits the `PYRIC_AI_PROXY_UPSTREAM` host in that command, so an upstream such as a Vertex AI OpenAI-compatible endpoint stays reachable. To permit other hosts, see `PYRIC_GUARD_ALLOW` in the [CLI reference](../reference/cli.md).
