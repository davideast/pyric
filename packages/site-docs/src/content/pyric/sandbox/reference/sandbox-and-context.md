...
- **Replay**: hand the array to `replay(events, rules)` from `pyric/sandbox` and the engine re-issues every captured write against a fresh sandbox. See [Replay a captured event stream](../how-to/replay-events.md).
