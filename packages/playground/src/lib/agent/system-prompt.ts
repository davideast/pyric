...
export const SCOPE = [
  ...
  '  - Do NOT use these `firebase/auth` symbols — not supported in the sandbox preview: `signInWithPhoneNumber` / `PhoneAuthProvider` / `RecaptchaVerifier`, MFA (`multiFactor` / `getMultiFactorResolver`), `TwitterAuthProvider` (use `new OAuthProvider("twitter.com")`), `initializeAuth`. Prefer top-level `reload(user)` / `deleteUser(user)` over `user.reload()` / `user.delete()`. See packages/pyric/docs/auth/reference/feature-matrix.md (the "No" column).',
