 ...
 * (plans/app-spec.md section 3). The fourth draft fence parses into this shape;
 * `deriveTests` (./derive.ts) turns it into workspace-test files the
 * host runs against the candidate rules.
 *
 * Schema mechanism (plans/app-spec.md section 5 "schema-first single
 ...
 *
 * Type cohesion (section 5, binding): the spec composes from EXECUTABLE types,
...
export const AccessRuleSchema = z.object({
  ...
  // The canonical method union — imported, never redeclared (section 5).
