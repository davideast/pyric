...
export function buildFirestoreExtractIndexesHandler(): ToolHandler {
  ...
  return {
    ...
    description:
      ...
      "Returns: `config` (ready to write to firestore.indexes.json for the Firebase CLI), per-collection `signals` (overshootSuspected = enumerated shapes > 3 → cue to add JSDoc `@firestore-mutex { a, b }` on the function), `annotationsApplied`, and per-file `warnings`. " +
